    ...
    <div class="tab-pane fade show active" id="pills-detail" role="tabpanel" aria-labelledby="pills-detail-tab">
			<form id="form_create_workpack" method="POST" action="<?php echo base_url() ?>planning/update_process_workpack_bnp">
						...
						<div class="card shadow my-3 rounded-0">
							...
							<div class="card-body bg-white overflow-auto">
								...
								<div class="form-group row">
									...
									<div class="col-md">
										<input type="number" class="form-control" value="<?php echo (isset($show_data_irn_list) ? count($show_data_irn_list) : count($detail_list))  ?>" readonly>
									</div>
								</div>
				...
				<div class="row">
					<div class="col">
						<div class="card shadow my-3 rounded-0">
							<div class="card-header" id="slide_down">
								<h6 class="m-0"><?php echo $meta_title ?> <i class="fas fa-angle-double-down"></i></h6>
							</div>
							<div class="card-body bg-white" id="form_data">
						...
						<div class="card shadow my-3 rounded-0">
							...
							<div class="card-body bg-white">
								...
								<div class="overflow-auto">
									...
									<?php $total_data_pc = sizeof($show_data_irn_list); ?>
										...
										<tbody>
											...
											foreach ($show_data_irn_list as $key => $value) { ?>
												...
												<tr>
													...
													<td><?= $value['part_id'] ?></td>
													<td>
														<?= (isset($data_submited[$value['id']]) ? "<span class='badge badge-primary'>" . $data_submited[$value['id']]["workpack_no"] . "</span>" : null) ?>
														...
														<select name='paint_system[<?= $value["id"] ?>][]' class='form-control select2_multiple_paint_system' multiple>
															<option value='N/A' <?= (isset($paint_system_capture_joint[$detail_list[0]["id_workpack"]][9999][$value['id']]["id_workpack"]) ? "selected"  : null) ?>>N/A</option>
															<?php $noxxx[$no] = 0;
															foreach ($get_paint_system as $keyx => $valx) { ?>
																<?php
																if (isset($paint_system_capture_joint[$detail_list[0]["id_workpack"]][$valx['id']][$value['id']]["id_workpack"]) || isset($paint_system_capture_joint[$detail_list[0]["id_workpack"]][9999][$value['id']]["id_workpack"])) {
																	$noxxx[$no]++;
																}
																?>
													...
													<td><textarea name='remarks_pc[<?= $no ?>]' class='form-control' placeholder='Remarks'><?= (isset($paint_system_remarks_template[$detail_list[0]["id_workpack"]][$value["id"]]["remarks"]) ? $paint_system_remarks_template[$detail_list[0]["id_workpack"]][$value["id"]]["remarks"] : null) ?></textarea></td>
...
<script type="text/javascript">
	...
	$('#slide_down').click(function() {
		$('#form_data').slideToggle(1000, function() {});
	});
</script>