...
<div id="content" class="container-fluid">
  ...
  <form action="<?= site_url('planning/save_to_material_new_reinstatement') ?>" method="post" onsubmit="if( _formConfirm_submitted == false ){ _formConfirm_submitted = true;return true }else{ alert('Please Wait, Server still busy, wait till process done, Thanks!'); return false;  }" enctype="multipart/form-data" >
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group row">
                      ...
                      <div class="col-md-8 col-lg-9">
                      <select class="select2" name="area" id='area' required>
                        <option value="">---</option>
                  ...
                  <tr>
                    ...
                    <td>
                        <?php if(strlen($value['evidence_mv'])>1){ ?>
                            ...
                            <?php  
                                $enc_redline = strtr($this->encryption->encrypt($value['evidence_mv']), '+=/', '.-~');
                    ...
                    <td>   
                      ...
                      <select  class='select2_multiple_itr editable' name='id_mis[<?= $key ?>][]' multiple disabled> 
                          <?php 
                            if(isset($value['id_mis_piping'])){

                                $data_list_uniq[$key] = explode(";",$value['id_mis_piping']);
                                foreach($data_list_uniq[$key] as $val_un){                                
                          ?>       
                                <option value='<?= $val_un ?>' selected><?= isset($mis_detail[$val_un]) ? $mis_detail[$val_un]['unique_no']." (". $mis_detail[$val_un]['description'] .")" : '' ?></option>
                          <?php  } } ?>                        
                      </select>
                      <input type="hidden" name="id_mis_material[<?= $key ?>]" value="<?= $value['id_mis_piping'] ?>">
                    ...
                    <td>
                      <?php
                      if(isset($value["progress_mv"])){
                        $progress = number_format($value["progress_mv"], 2);
                      } else{
...
$(document).ready(function() {
    $(".select2_multiple_itr").select2({ 
          tokenSeparators: [',', ' '],
          ajax: {
                url: "<?php echo base_url();?>material_verification/autocomplete_unique_no_for_piping",
                type: "post",
                dataType       : 'json',
                data: function (params) {
                  var query = {
                      search: params.term,
                      workpack_no: '<?= $workpack['workpack_no'] ?>', 
                      id_workpack: '<?= $workpack['id'] ?>',  
                  }
                  return query;
                },
