...
<div id="content" class="container-fluid">
	...
	<form action="<?= site_url('planning/save_to_material_new') ?>" method="post" onsubmit="if( _formConfirm_submitted == false ){ _formConfirm_submitted = true;return true }else{ alert('Please Wait, Server still busy, wait till process done, Thanks!'); return false;  }" enctype="multipart/form-data">
							...
							<div class="row">
								...
								<div class="col-md-6">
									<div class="form-group row">
										...
										<div class="col-md-8 col-lg-9">
											<input type="file" name="attachment_surveyor_mv" required class='editable'>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="card-body bg-white">
						<div class="overflow-auto">
							<table class="table table-hover text-center dataTable" id='table_submission'>
								...
								<tbody>
									<?php $no_submitted = 0;
									foreach ($piecemark_list as $key => $value) :  if (!isset($value['id_piecemark'])) {
											$no_submitted++;
										}
										if ($value['report_resubmit_status'] == 0 or !isset($value['report_resubmit_status'])) { ?>
											<tr>
												<td>
													<?php if (!isset($value['id_piecemark'])) : ?>
														<input type="hidden" name="unique_no[<?= $key ?>]" class="unique_no" value="<?= $value['unique_no'] ?>">
														<input type="hidden" name="id_mis[<?= $key ?>]" class="id_mis" value="<?= $material_mis_list[$value['unique_no']] ?>">
