<?php 
  ...
  if(count($list_id_foreman) >= 3) {
    $col_width  = 4;
  } elseif(count($list_id_foreman) == 2) {
    $col_width  = 6;
  } elseif(count($list_id_foreman) == 1) {
    $col_width  = 12;
  }
...
<div class="row">
  <?php foreach ($list_id_foreman as $key => $value) : ?>

    <div class="col-md-<?= $col_width ?>">
      <div id="foreman_weekly_<?= $key ?>" style="width: 100%; height: 100%"></div>
    </div>
  <?php endforeach; ?>
</div>
...
<script>
  ...
  <?php foreach ($list_id_foreman as $key => $value) : ?>
    ...
    var chart = new Highcharts.Chart({
      chart: {
        renderTo: 'foreman_weekly_<?= $key ?>',
      },
      title: {
        text: ` Foreman - <strong><?= $user[$value]['full_name'] ?></strong>`
      },
