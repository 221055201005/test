...
<style>
    ...
    td:nth-child(n+9):nth-child(-n+21) {
...
<html lang="en">
 ...
 <body>
            ...
            <?php $no_max = sizeof($show_data_irn_list_filter); $no=1; foreach($show_data_irn_list_filter as $key => $loop_val){ ?>
            ...
            <table class="wtr">
              <thead>  
                <tr>
                  ...
                  <th colspan="23" style="padding:2px !important;">Non Destructive Examination</th>
              ...
              <tbody>
                ...
                $array_qc_approval = array(0,1,2,4,6,8,9,10,11);
                  ...
                  foreach ($wtr_list as $key => $value) {
                    ...
                    foreach($pos_1 as $keypc1 => $pc1){ 
                      $pc_pos_1[$keypc1]           = @$status_piecemark[$pc1];
                      $report_enc_mv_p1[$keypc1]   = null; 
                      $report_no_rev_p1[$keypc1]   = null; 
                      if(isset($pc_pos_1[$keypc1]['report_number'])){ 
                        $report_enc_mv_p1[$keypc1]   = strtr($this->encryption->encrypt($pc_pos_1[$keypc1]['report_number']), '+=/', '.-~'); 
                        $report_no_rev_p1[$keypc1]   = strtr($this->encryption->encrypt($pc_pos_1[$keypc1]['report_no_rev']), '+=/', '.-~'); 
                      }
                    }

                    $pos_2  = explode(";",$value['pos_2']); 
                    foreach($pos_2  as $keypc2 => $pc2){ 
                      $pc_pos_2[$keypc2]           = @$status_piecemark[$pc2]; 
                      $report_enc_mv_p2[$keypc2]   = null; 
                      $report_no_rev_p2[$keypc2]   = null;  
                      if(isset($pc_pos_2[$keypc2]['report_number'])){  
                        $report_enc_mv_p2[$keypc2]   = strtr($this->encryption->encrypt($pc_pos_2[$keypc2]['report_number']), '+=/', '.-~'); 
                        $report_no_rev_p2[$keypc2]   = strtr($this->encryption->encrypt($pc_pos_2[$keypc2]['report_no_rev']), '+=/', '.-~');
                      } 
                    }
                    ...
                    <tr>
                        ...
                        <?php 
                         ...
                         foreach($pos_1 as $keypc1 => $pc1){ 
                          ...
                          if(isset($pc_pos_1[$keypc1]['report_number']) AND $pc_pos_1[$keypc1]['status_inspection'] >= 3){ 
                        ?>
                              <?php 
                                $discipline_mv_enc      = encrypt($pc_pos_1[$keypc1]['discipline']);
                                $project_mv_enc         = encrypt($pc_pos_1[$keypc1]['project_code']);
                                $module_mv_enc          = encrypt($pc_pos_1[$keypc1]['module']);
                                $type_module_mv_enc     = encrypt($pc_pos_1[$keypc1]['type_of_module']);
                                $report_number_mv_enc   = encrypt($pc_pos_1[$keypc1]['report_number']);
                                $report_rev_enc         = encrypt($pc_pos_1[$keypc1]['report_no_rev']);
                                $drawing_mv_enc         = encrypt($pc_pos_1[$keypc1]['drawing_no']);
                                ...
                                $report_number_1  = @$report_no_mv[$value['project']][$pc_pos_1[$keypc1]['discipline']][$value['type_of_module']]['mv_no'.($status_piecemark[$pc1]["company_id"]==13 ? '_smop' : '')]."-".$pc_pos_1[$keypc1]['report_number'];
                              ...
                              <a <?= ($pc_pos_1[$keypc1]['status_inspection'] == 5 ? "class='color_pending_client'" : null) ?> href="<?= $link_report_1 ?>" target='_blank'><?= $report_number_1.'' ?></a>  <hr/>

                              <?php } else { ?>
                                <?php 

                                $report_number_1    = "-";
                                if($pc_pos_1[$keypc1]['report_number']) {
                                  $report_number_1  = @$report_no_mv[$keypc1][$value['project']][$pc_pos_1[$keypc1]['discipline']][$value['type_of_module']]['mv_no'.($status_piecemark[$pc1]["company_id"]==13 ? '_smop' : '')]."-".$pc_pos_1[$keypc1]['report_number'];   
                                }

                                $link_report_1        = base_url()."material_verification/material_verification_pdf/".encrypt($pc_pos_1[$keypc1]['submission_id']); 
                                  
                                ?>

                                <?php if($pc_pos_1['status_inspection'] == 1){ ?>
                                  <a href="<?= $link_report_1 ?>" target='_blank'><?= $pc_pos_1[$keypc1]['submission_id'] ?></a>  <hr/>
                        ...
                        <td style="padding:2px !important;">
                            ...
                            <?php 
                            foreach($pos_1 as $keypc1 => $pc1){ 
                              if(isset($pc_pos_1[$keypc1]['id_mis'])){ 

                                // echo "<a target='_blank' href='".$server_warehouse."/mrir/export_mrir_cs/?id=".strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['mrir_id']),'+=/', '.-~').'&action='.strtr($this->encryption->encrypt('update'),'+=/', '.-~').'&status='.strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['status']),'+=/', '.-~').'&partial_report_no='.strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['partial_report_no']),'+=/', '.-~')."'>"."<span class='badge'>".$warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['unique_ident_no']."</span></a><br/><hr/>"; 

                                $unique_disc            = $warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['discipline'];
                                $mrir_id                = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['mrir_id']), '+=/', '.-~');
                                $status                 = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['status']), '+=/', '.-~');
                                $partial_report_no      = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['partial_report_no']), '+=/', '.-~');
                                ...
                                echo "<a target='_blank' href='" . $link_mrir2 . "'>" . "<span class='badge'>" . $warehouse_mis_mrir[$pc_pos_1[$keypc1]['id_mis']]['unique_ident_no'] . "</span></a><br/><hr/>";
                        ...
                        <?php 
                         ...
                         foreach($pos_2 as $keypc2 => $pc2){ 
                          ...
                          if(isset($pc_pos_2[$keypc2]['report_number']) AND $pc_pos_2[$keypc2]['status_inspection'] >= 3){ 
                        ?>

                              <?php 
                                
                                $discipline_mv_enc      = encrypt($pc_pos_2[$keypc2]['discipline']);
                                $project_mv_enc         = encrypt($pc_pos_2[$keypc2]['project_code']);
                                $module_mv_enc          = encrypt($pc_pos_2[$keypc2]['module']);
                                $type_module_mv_enc     = encrypt($pc_pos_2[$keypc2]['type_of_module']);
                                $report_number_mv_enc   = encrypt($pc_pos_2[$keypc2]['report_number']);
                                $report_rev_enc         = encrypt($pc_pos_2[$keypc2]['report_no_rev']);
                                $drawing_mv_enc         = encrypt($pc_pos_2[$keypc2]['drawing_no']);
                                ...
                                $report_number_2  = @$report_no_mv[$value['project']][$pc_pos_2[$keypc2]['discipline']][$value['type_of_module']]['mv_no'.($status_piecemark[$pc1]["company_id"]==13 ? '_smop' : '')]."-".$pc_pos_2[$keypc2]['report_number'];
                              ...
                              <a <?= ($pc_pos_2[$keypc2]['status_inspection'] == 5 ? "class='color_pending_client'" : null) ?> href="<?= $link_report_2 ?>" target='_blank'><?= $report_number_2.'' ?></a>  <hr/>

                              <?php } else { ?>
                                <?php 

                                $report_number_2    = "-";
                                if($pc_pos_2[$keypc2]['report_number']) {
                                  $report_number_2  = @$report_no_mv[$keypc2][$value['project']][$pc_pos_2[$keypc2]['discipline']][$value['type_of_module']]['mv_no'.($status_piecemark[$pc2]["company_id"]==23 ? '_smop' : '')]."-".$pc_pos_2[$keypc2]['report_number'];   
                                }

                                $link_report_2        = base_url()."material_verification/material_verification_pdf/".encrypt($pc_pos_2[$keypc2]['submission_id']); 
                                  
                                ?>

                                <?php if($pc_pos_2['status_inspection'] == 2){ ?>
                                  <a href="<?= $link_report_2 ?>" target='_blank'><?= $pc_pos_2[$keypc2]['submission_id'] ?></a>  <hr/>
                        ...
                        <td style="padding:2px !important;">
                            ...
                            <?php 
                            foreach($pos_2 as $keypc2 => $pc2){ 
                              if(isset($pc_pos_2[$keypc2]['id_mis'])){ 
                                // echo "<a target='_blank' href='".$server_warehouse."/mrir/export_mrir_cs/?id=".strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['mrir_id']),'+=/', '.-~').'&action='.strtr($this->encryption->encrypt('update'),'+=/', '.-~').'&status='.strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['status']),'+=/', '.-~').'&partial_report_no='.strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['partial_report_no']),'+=/', '.-~')."'>"."<span class='badge'>".$warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['unique_ident_no']."</span></a><br/><hr/>"; 

                                $unique_disc            = $warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['discipline'];
                                $mrir_id                = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['mrir_id']), '+=/', '.-~');
                                $status                 = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['status']), '+=/', '.-~');
                                $partial_report_no      = strtr($this->encryption->encrypt($warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['partial_report_no']), '+=/', '.-~');
                                $action_enc             = strtr($this->encryption->encrypt("detail"), '+=/', '.-~');
                                ...
                                echo "<a target='_blank' href='" . $link_mrir2 . "'>" . "<span class='badge'>" . $warehouse_mis_mrir[$pc_pos_2[$keypc2]['id_mis']]['unique_ident_no'] . "</span></a><br/><hr/>";
                        ...
                        <td><?php if($value['visual_ndt_pwht'] == 1){ echo "YES"; } else { echo "-"; } ?></td>    
                        <td><?php if($value['visual_ndt_pwht'] == 0){ echo "NO"; } else { echo "-"; } ?></td>  
                        <td style="padding:2px !important;"> 
                            <?php
                ...
                <?php if($is_wtr!='wtr'){ ?>  
                  <table class="table-body" width="100%" style="text-align: left; border-collapse: collapse !important;">
                    <tr>
                      ...
                      <td style="width: 25%; border: none; padding-top: 10px;"><center><b>EMPLOYER</b></center></td>
                      <td style="width: 25%; border: none; padding-top: 10px;"><b></b></td>
