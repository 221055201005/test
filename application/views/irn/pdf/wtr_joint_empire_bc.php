...
<body>
  ...
  foreach ($show_data_irn_list_filter as $key => $loop_val) { ?>
    ...
    <table class="wtr">
      <thead>
        <tr>
          <th rowspan="3" style="width: 50px !important;"><br /><br />Drawing/Weld Map No</th>
          ...
          <?php if ($wtr_list[0]['discipline'] == 1) { ?>
            <th rowspan="3" style="width:50px !important" style="padding:2px !important;">Test Package Number</th>
          <?php } ?>
      ...
      <tbody>
        ...
        ?>
          ...
          <tr>
            ...
            <td style="padding:2px !important;" <?= (in_array($pc_pos_1['status_inspection'], $array_qc_approval) && isset($pc_pos_1['status_inspection']) ? "class='color_pending_QC'" : null) ?>>
              ...
              <?php } else { ?>
                ...
                <?php if (isset($pc_pos_1['report_number']) and $pc_pos_1['status_inspection'] >= 3) { ?>
                  ...
                  $report_number_1  = @$report_no_mv_deck[$value['company_id']][$value['project']][$pc_pos_1['discipline']][$value['type_of_module']][$value['deck_elevation']]['mv_no' . ($status_piecemark[$value["pos_1"]]["company_id"] == 13 ? '_smop' : '')] . "-" . $pc_pos_1['report_number'];
            ...
            <td style="padding:2px !important;" <?= (in_array($pc_pos_2['status_inspection'], $array_qc_approval) && isset($pc_pos_2['status_inspection']) ? "class='color_pending_QC'" : null) ?>>
              ...
              <?php } else { ?>
                ...
                <?php if (isset($pc_pos_2['report_number']) and $pc_pos_2['status_inspection'] >= 3) { ?>
                  ...
                  $report_number_2  = @$report_no_mv_deck[$value['company_id']][$value['project']][$pc_pos_1['discipline']][$value['type_of_module']][$value['deck_elevation']]['mv_no' . ($status_piecemark[$value["pos_2"]]["company_id"] == 13 ? '_smop' : '')] . "-" . $pc_pos_2['report_number'];
            ...
            <td style="padding:2px !important;" <?= ($this->permission_cookie[0] == 1 || $this->permission_cookie[156] == 1 ? (date("Y-m-d", strtotime($value['visual_inspection_datetime'])) < date("Y-m-d", strtotime($value['weld_datetime'])) && isset($value['visual_inspection_datetime']) ? "class='color_date'" : null) : null) ?>><?php echo ($value['company_id'] == 5 ? date("Y-m-d", strtotime($value['weld_datetime'])) : date("Y-m-d", strtotime($value['weld_datetime']))); ?></td>
            ...
            <td style="padding:2px !important;">
              ...
              if (sizeof($welder_rh) > 0) {
                foreach ($welder_rh as $key => $welder_id_rh) {
                  if (isset($master_welder[$welder_id_rh]["welder_code"])) {
                    echo $master_welder[$welder_id_rh]["welder_code"] . "<br/>";
                  }
                }
              }
              ?>
            ...
            <td>
              <?php if ($value['visual_status_inspection'] != 5 && isset($value['visual_report_no'])) { ?>
                <?php 
                  
                  $report_vs = $report_no_vs[$value['project']][$value['discipline']][$value['type_of_module']]['visual_report' ] . $value['visual_report_no'];
                  if($value['project'] == 21) {
                    $report_vs = $report_no_vs_deck[$value['company_id']][$value['project']][$value['discipline']][$value['type_of_module']][$value['deck_elevation']]['visual_report' ] . $value['visual_report_no'];
                  }
            ...
            <td style="padding:2px !important;">
              <?php if (isset($ndt_wtr_empire[$value["id_visual"]][4]["report_number"])) { ?>
                ...
                <a href="<?= site_url("ndt_live/ndt_detail_atc" . "/" . encrypt($ndt_wtr_empire[$value["id_visual"]][4]['submission_id']) . '/' . ($ndt_wtr_empire[$value["id_visual"]][4]['ndt_type']) . '/' . encrypt('pdf')); ?>" target="_blank"><?= $ndt_wtr_empire[$value["id_visual"]][4]["report_number"] ?></a>
              <?php
              } else {
                echo "-";
              }
