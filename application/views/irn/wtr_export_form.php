...
<form action="<?php echo base_url(); ?>irn/submit_irn_fab" enctype="multipart/form-data" method='POST' id="form_submition">
        ...
        <div class="my-3 p-3 bg-white rounded shadow-sm">
          ...
          <div class="overflow-auto media text-muted py-3 mt-1 border-bottom border-top border-gray">
            ...
            <div class="container-fluid">
              ...
              <div class="row">
                <div class="col-md">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <select class="form-control project2" name="project_joint" id="project2" required="" onchange="populateModuleChained(this);">
              ...
              <div class="row <?= $get['project_joint'] == 21 ? null : 'd-none' ?>" id="div_deck">
                <div class="col-md">
                  <div class="form-group row">
                    <label class="col-xl-3 col-form-label"> Deck Elevation / Service Line :</label>
                    <div class="col-xl">
                      <select class="form-control" name="deck_elevation" id="deck_change">
                        <option value="">---</option>
                  ...
                  <div class="form-group row">
                    <div class="col-xl text-left">
                      ...
                      <?php if ($is_bondstrand != 1) { ?>
                        <a href="<?= base_url("irn/import_joint_irn") ?>" class="btn btn-success"><i class="fas fa-file-excel"></i>
                          Import Joint
                        </a>
                      <?php } ?>
        ...
        <div class="my-3 p-3 bg-white rounded shadow-sm">
          ...
          <div class="overflow-auto media text-muted py-3 mt-1 border-bottom border-gray">
            <div class="container-fluid">
              ...
              <table class="table" width="100%" id='tableListJoint'>
                <thead>
                  <tr>
                    ...
                    <th rowspan='2'>Fit-Up<br />Status</th>
                    <th rowspan='2'>Visual<br />Status</th>
                    <th colspan='4' style='text-align:center;'>NDT Status</th>
                ...
                <tbody>
                  <?php if (isset($data_joint_list)) { ?>
                    ...
                    <?php foreach ($data_joint_list as $key => $value) { ?>
                      ...
                      ?>
                      <?php
                      $status_MT_popupated = array();
                      $status_PT_popupated = array();
                      $status_UT_popupated = array();
                      $status_RT_popupated = array();
                      ...
                      if ($value['status_inspection_fitup'] == 7) {
                        $status_fitup = "COMPLETED";
                      } else {
                        $status_fitup = 'OS';
                      }

                      if ($value['status_inspection_visual'] == 7) {
                        $status_visual = "COMPLETED";
                      } else {
                        $status_visual = 'OS';
                      }

                      if (isset($ndt_all[$value['id']][2])) {
                        foreach ($ndt_all[$value['id']][2] as $key => $valuex1) {
                          if ($valuex1["result"] == '3') {
                            $status_MT_popupated[] = "COMPLETED";
                          } else {
                            $status_MT_popupated[] = "OS";
                          }
                        }
                        $status_MT_show[$key] = implode("<hr/>", $status_MT_popupated);
                      } else {
                        if (isset($value['mt_percent_req']) && !empty($value['mt_percent_req'])) {
                          $status_MT_show[$key] = "OS";
                        } else {
                          $status_MT_show[$key] = "-";
                        }
                      }
                      ...
                      if (isset($ndt_all[$value['id']][7])) {
                        foreach ($ndt_all[$value['id']][7] as $key => $valuex2) {
                          if ($valuex2["result"] == '3') {
                            $status_PT_popupated[] = "COMPLETED";
                          } else {
                            $status_PT_popupated[] = "OS";
                          }
                        }
                        $status_PT_show = implode("<hr/>", $status_PT_popupated);
                      } else {
                        if (isset($value['pt_percent_req']) && !empty($value['pt_percent_req'])) {
                          $status_PT_show = "OS";
                        } else {
                          $status_PT_show = "-";
                        }
                      ...
                      if (isset($ndt_all[$value['id']][1])) {
                        foreach ($ndt_all[$value['id']][1] as $key => $valuex3) {
                          if ($valuex3["result"] == '3') {
                            $status_RT_popupated[] = "COMPLETED";
                          } else {
                            $status_RT_popupated[] = "OS";
                          }
                        }
                        $status_RT_show = implode("<hr/>", $status_RT_popupated);
                      } else {
                        if (isset($value['rt_percent_req']) && !empty($value['rt_percent_req'])) {
                          $status_RT_show = "OS";
                        } else {
                          $status_RT_show = "-";
                        }
                      ...
                      if (isset($ndt_all[$value['id']][3])) {
                        foreach ($ndt_all[$value['id']][3] as $key => $valuex4) {
                          if ($valuex4["result"] == '3') {
                            $status_UT_popupated[] = "COMPLETED";
                          } else {
                            $status_UT_popupated[] = "OS";
                          }
                        }
                        $status_UT_show = implode("<hr/>", $status_UT_popupated);
                      } else {
                        if (isset($value['rt_percent_req']) && !empty($value['rt_percent_req'])) {
                          $status_UT_show = "OS";
                        } else {
                      ...
                      ?>
                      <tr class="tr_<?= $value['id_irn'] ?>">
                        <td>
                          <input type="checkbox" class="id_irn" name="id_irn[]" value="<?= $value['id_irn'] ?>">
                        </td>
                        ...
                        <td><?= $thickness_p1 ?>
                          <hr /><?= $thickness_p2 ?></td>
                        <td><?= $status_inspection_p1 ?>
                          <hr /><?= $status_inspection_p2 ?></td>
                        <td><?= $status_fitup ?></td>
                        <td><?= $status_visual ?></td>
                        <td><?= $status_MT_show[$key] ?></td>
                        <td><?= $status_PT_show ?></td>
                        <td><?= $status_UT_show ?></td>
                        <td><?= $status_RT_show ?></td>
                        <td>
              ...
              <?php if (isset($data_joint_list)) { ?>
                <span onclick="remove_joints(this)" class='btn btn-danger'>
                  Delete
                </span>
              <?php } ?>
            </div>
                    ...
                    <div class="col-xl">
                      <select class="select2 will_enable" name="area_v2" required>
                        ...
                        <?php foreach ($area_name_list_v2 as $value_area) { ?>
                          <option value="<?= $value_area['id'] ?>" <?php if ($show_pcms_irn[0]['area_v2'] == $value_area['id']) {
                                                                      echo "selected";
                    ...
                    <div class="col-xl">
                      <select class="select2 will_enable" name="location_v2" required>
                        ...
                        <?php foreach ($location_name_list_v2 as $value_location) { ?>
                          <option value="<?= $value_location['id'] ?>" data-chained="<?php echo $value_location['id_area'] ?>" <?php if ($show_pcms_irn[0]['location_v2'] == $value_location['id']) {
                                                                                                                                  echo "selected";
                ...
                <tbody>
                  <?php if (isset($show_pcms_irn_dc)) { ?>
                    <?php foreach ($show_pcms_irn_dc as $key => $value) { ?>
                      <tr>
                        <td>
                          <center><?= $data_dc_show[$value['id_detail_dimension']]['detail_report_number'] ?></center>
                        </td>
                        <td>
                          <center>
                            <?php
                ...
                <tbody>
                  <?php if (isset($show_pcms_irn_pnc)) { ?>
                    <?php foreach ($show_pcms_irn_pnc as $key => $value) { ?>
                      <tr>
                        <td>
                          <center><?= $data_punc_show[$value['id_detail_dimension']]['report_number'] ?></center>
                        </td>
                        <td>
                          <center>
        ...
        ajax: {
          ...
          data: function(params) {
            var query = {
              ...
              deck_elevation: $("select[name='deck_elevation']").find('option:selected').val(),
    ...
    function autodrawingByjoint(input) {
      ...
      $(input).autocomplete({
        source: function(request, response) {
          $.post('<?php echo base_url(); ?>wtr/display_drawing', {
            term: request.term,
      ...
      $(input).autocomplete({
        source: function(request, response) {
          $.post('<?php echo base_url(); ?>wtr/display_drawing', {
            ...
            module_value: module_value,
            discipline_value: discipline_value,
            drawing_ga: drawing_joint_value
          }, response, 'json');
        },
        autoFocus: true,
        classes: {
  ...
  <script type="text/javascript">
    ...
    function populateModuleChained(select) {
    ...
    function auto_joint_number(input) {
      ...
      $(input).autocomplete({
        source: function(request, response) {
          $.post('<?php echo base_url(); ?>irn/display_joint_number', {
            term: request.term,
...
<script type="text/javascript">
  ...
  function remove_joints(ini) {
    var id_irn_arr = []
    $('.id_irn').map(
      (i, v) =>
      v.checked ? id_irn_arr.push(v.value) : ''
    )
    console.log(id_irn_arr)

    Swal.fire({
      title: 'Are you sure to Remove these Joints!',
      ...
      if (result.value) {
        $.ajax({
          url: "<?= base_url('irn/remove_joints/') ?>",
          type: "post",
          data: {
            'id_irn_arr': id_irn_arr,
          },
          success: function(data) {
            Swal.fire(
