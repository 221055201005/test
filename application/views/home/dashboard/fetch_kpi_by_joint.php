  ...
  <table border="1" class="table table-hover table-bordered text-center">
    <tbody>
      ...
      <tr class="alert-success">
        ...
        <th colspan="4">MPI Status Based on Total Joint Required MT</th>
        <th colspan="4">UT Status Based on Total Joint Required UT</th>
      </tr>
      <tr class="alert-success">
      ...
      <?php foreach ($deck_list as $value) : ?>
        ...
        if (in_array($category, ['special','primary'])) {
          $need_mt = floor(intval($join_list[$value['id']]['total_joint']));
        } elseif ($category == "secondary") {
          $need_mt = floor((intval($join_list[$value['id']]['total_joint']) * 5) / 100);
        } else {
          $need_mt = floor((intval($join_list[$value['id']]['total_joint']) * 20) / 100);
        }
        ...
        <tr>
          <td><?= $value['name'] ?></td>
          <td><?= intval($join_list[$value['id']]['total_joint']) ?></td>
          <td><?= intval($fitup[$value['id']]['total_joint_submission']) ?></td>
          <td><?= intval($fitup[$value['id']]['total_joint_approved']) ?></td>
          <td><?= intval($fitup[$value['id']]['total_joint_reject']) ?></td>
          <td>
            <?= intval($join_list[$value['id']]['total_joint']) - intval($fitup[$value['id']]['total_joint_approved']) ?>
          </td>
          <td>
            <?php

            $percentage_fitup = (intval($fitup[$value['id']]['total_joint_approved']) / intval($join_list[$value['id']]['total_joint'])) * 100;
          ...
          <td><?= intval($visual[$value['id']]['total_joint_submission']) ?></td>
          <td><?= intval($visual[$value['id']]['total_joint_approved']) ?></td>
          <td><?= intval($visual[$value['id']]['total_joint_reject']) ?></td>
          <td>
            <?= intval($join_list[$value['id']]['total_joint']) - intval($visual[$value['id']]['total_joint_approved']) ?>
          </td>
          <td>

          <?php

            $percentage_visual = (intval($visual[$value['id']]['total_joint_approved']) / intval($join_list[$value['id']]['total_joint'])) * 100;
          ...
          <td><?= intval($visual[$value['id']]['total_joint_all']) ?></td>
          <?php foreach ($joint_type as $v) : ?>
          ...
          <td>
            <?= intval($ndt_count['2'][$value['id']]['total']) ?>
          </td>
          <td>
            <?= intval($need_mt - $ndt_count['2'][$value['id']]['total']) > 0 ? intval($need_mt - $ndt_count['2'][$value['id']]['total']) : 0 ?>
          </td>
          <td>

            <?php

            $percentage_mt = ($ndt_count['2'][$value['id']]['total'] / $need_mt) * 100;
          ...
          <td><?= $need_ut ?></td>
          <td>
            <?= intval($ndt_count['3'][$value['id']]['total']) ?>
          </td>
          <td>
            <?= intval($need_ut - $ndt_count['3'][$value['id']]['total']) > 0 ? intval($need_ut - $ndt_count['3'][$value['id']]['total']) : 0 ?>
          </td>
          <td>

            <?php

            $percentage_ut = ($ndt_count['3'][$value['id']]['total'] / $need_ut) * 100;
