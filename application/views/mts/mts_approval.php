...
if (in_array($status, ["0", "ready_transmit"]) && $this->input->get('drawing_no')) {
  $show_check = true;
  if(in_array($this->input->get('project_id'), project_by_deck()) && $this->input->get('deck_elevation') == '') {
    $show_check = false;
  }
}
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group row">
                        ...
                        <div class="col-xl">
                          <select name="project_id" class="select2" onchange="get_module(this)" style="width:100%" required>
                            <option value="">---</option>
                        ...
                        <div class="col-xl">
                          <select name="module" class="select2" style="width:100%">
                            ...
                            <?php if ($this->input->get('project_id')): ?> 
                              <?php foreach ($module_list[$this->input->get('project_id')] as $key => $value) : ?>
                        ...
                        <div class="col-xl">
                          <select name="deck_elevation" class="select2" style="width:100%">
                            ...
                            <?php foreach ($deck as $key => $value) : ?>
                              <option value="<?= $value['id'] ?>" <?= $value['id'] == $this->input->get('deck_elevation') ? 'selected' : '' ?>><?= $value['name'] ?></option>
                            <?php endforeach; ?>
                          </select>
                        </div>
                      </div>
    ...
    <div class="row">
      <div class="col-md-12 my-3">
        <div class="card shadow">
          <div class="card-header">
            <h6 class="card-title m-0"> MTS Approval List</h6>
          </div>
          <div class="card-body">
            <?php if ($status == "0") : ?>
              <form id="form_submit" action="<?= site_url('mts/submit_data_mts_signed') ?>" method="post">
              <?php else : ?>
                <form id="form_submit" action="<?= site_url('mts/transmit_data_mts_signed') ?>" method="post">
                <?php endif; ?>
                <input type="hidden" name="deck_elevation" value="<?= $this->input->get('deck_elevation') ?>">
                <div class="row">
                  <div class="col-md-12">
                        ...
                        <tbody>
                          <?php foreach ($list as $key => $value) : ?>
                            ...
                            <tr>
                              ...
                              <td><?= $deck[$pc[$value['id_piecemark']]['deck_elevation']]['name'] ?></td>
                              <td>
                              ...
                              <td>
                                <div class="btn-group">
                                  <?php if ($value['status_inspection'] == 1 && in_array($this->user_cookie[0], $this->allowed_user)) : ?>
                                    <button type="button" onclick="return_mts(this, '<?= encrypt($value['submission_id']) ?>')" type="button" class="btn btn-warning"> <i class="fas fa-undo"></i> Return to Draft</button>
                                  <?php endif; ?>
                                  <a href="<?= site_url('mts/detail_mts_signed/' . encrypt($value['uniq_id'])) ?>" class="btn btn-primary"><i class="fas fa-list"></i> Detail</a>
                                </div>
                              </td>
...
<script>
  ...
  function return_mts(btn, enc_submission_id) {
    Swal.fire({
      type: "warning",
      ...
      if (res.value) {
        ...
        $.ajax({
          url: "<?= site_url('mts/return_to_draft') ?>",
          type: "POST",
          data: {
