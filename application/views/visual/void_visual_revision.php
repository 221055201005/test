    ...
    <div class="col">
      <div class="card shadow my-3 rounded-0">
        ...
        <div class="card-body bg-white">
          <form method="POST" action="<?= base_url() ?>visual/submit_void_visual_revision" id="form_submition">
            <br>
              ...
              <table class="table table-hover text-center dataTable" width="100%">
                <thead class="bg-green-smoe text-white">
                  <tr>
                    ...
                    <th rowspan="2">Status Inspection</th>
                    ...
                    <th rowspan="2" style="width: 200px !important">Search Joint</th>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td style="vertical-align: middle;">
                      <div class="custom-control custom-checkbox mr-sm-2 div_<?= $key ?>">
                        ...
                        <input disabled class="form-control will_enable<?= $key ?>" type="hidden" name="id_joint[<?= $key ?>]" value="<?= $value['id_joint'] ?>">
                        <input disabled class="form-control will_enable<?= $key ?>" type="hidden" name="joint_no[<?= $key ?>]" value="<?= $value['joint_no'] ?>">
                    ...
                    <td>
                        <?php if (@$value['status_inspection'] == "0") : ?>
                          <span class="badge badge-pill badge-info">Submit RFI</span>
                        ...
                        <?php elseif (@$value['status_inspection'] == 5) : ?>
                          <span class="badge badge-pill badge-warning">Pending by Client</span>
                        ...
                        <?php elseif (@$value['status_inspection'] == 7) : ?>
                          <span class="badge badge-pill bg-teal text-white">Approved By Client</span>
                    ...
                    <td><?= $value['submission_id'] != NULL ? $value['submission_id'] : '-' ?></td>
                    <td>
                      <?php if ($value['project'] == 21) {
                          $reno_view = $this->master_report_no[$value['project']][$value['company_id']][$value['discipline']][$value['module']][$value['type_of_module']][$value['deck_elevation']]['visual_report'];
                        } else {
                          $reno_view = $this->master_report_no[$value['project']][$value['company_id']][$value['discipline']][$value['module']][$value['type_of_module']]['visual_report'];
                        } ?>
                        <?= $value['report_number'] ? $reno_view . $value['report_number'] : '-' ?>
                    </td>
                    ...
                    <td>
                      <?php
                        $drawing_wm = preg_replace('/\s/', '', $value['drawing_wm']);
                        $joint_no   = explode('#', $value['joint_no']);
                        $joint_no   = implode('%23', $joint_no);
                       ?>
                      <a target='_blank' href='<?= base_url() ?>engineering/search_joint?drawing_wm=<?= $drawing_wm ?>&spool_no=<?= $value['spool_no'] ?>&joint_no=<?= $joint_no ?>'><i class='fas fa-paperclip'></i> Open Joint </a>
                    </td>
                  </tr>
                  <?php endforeach; ?>
                </tbody>
