<form action="<?= site_url('fitup/add_joint_on_report_process') ?>" method="post">
  <input type="hidden" name="report_number" value="<?= $report_number ?>">
  <input type="hidden" name="id_fitup_ex" value="<?= $id_fitup_ex ?>">
  <div class="row">
    <div class="col-md-12">
    	...
    	<table class="table">
    		...
    		<tbody>
  			<?php foreach ($joint_list as $key => $value) { ?>
  				<tr>
  					<td>
  						<input type="checkbox" name="id_fitup[]" value="<?= $value["id_fitup"] ?>">
  						<?= $value["joint_no"] ?>
  					</td>
