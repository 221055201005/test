...
<form method="POST" action="<?php echo base_url();?>fitup/proses_request_update_report_number" id='form_submition'>
              ...
              <div class="row">
                <div class="col-md">
                 ...
                 <div class="col-md">
                  <div class="form-group">
                    <label>Area</label>
                        <select class="select2 select_area" id= 'mySelect' <?= $user_permission[31] == 1 ? 'onchange="change_area(this)"' : 'disabled' ?>>
              ...
              <div class="row">
                <div class="col-md">
                  <div class="form-group">
                    ...
                    <input type="text" id="datepicker" class="form-control" name="inspection_datetime" value="<?php echo date('Y-m-d', strtotime($joint_list[0]["inspection_datetime"])) ?>" >
                  </div>
                </div>
                 <div class="col-md">
                  <div class="form-group">
              ...
              <div class="row">
                <div class="col-md">
                  <div class="form-group">
                    ...
                    <textarea name='latest_update_remarks' class='form-control' placeholder='Remarks For Update' required></textarea>
                  </div>
                </div>
                 <div class="col-md">
                  <div class="form-group">
        ...
        <div class="card-body bg-white overflow-auto">
          <div class="text-right <?= $revise_history["status_revise"]!=1 ? 'd-none' : '' ?>">
          	<span class="btn btn-warning" onclick="add_joint_on_report(this, 
          		'<?= $joint_list[0]['drawing_no'] ?>', 
          		'<?= $joint_list[0]['report_number'] ?>',
          		'<?= $joint_list[0]['discipline'] ?>',
							'<?= $joint_list[0]['module'] ?>',
							'<?= $joint_list[0]['type_of_module'] ?>',
							'<?= $joint_list[0]['id_fitup'] ?>'
          	)">
          ...
          <table class="table table-hover text-center overflow-auto dataTable">
              ...
              <tbody>
                 ...
                 <tr>
                  ...
                  <td>
                        <select  class='select2_multiple_fitter' name='fitter_id[<?php echo $no; ?>][]' multiple required disabled>
                          <?php
             ...
             <div class="text-right mt-3">
                ...
                <a href='<?php echo  base_url(); ?>fitup/pdf_files/<?php echo strtr($this->encryption->encrypt($joint_list[0]['project_code']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['discipline']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['module']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['type_of_module']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['report_number']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['company_id']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['company_id']),'+=/', '.-~'); ?>/<?php echo strtr($this->encryption->encrypt($joint_list[0]['company_id']),'+=/', '.-~'); ?>' target='_blank'><button class='btn btn-danger' type="button"><i class="fas fa-file-pdf"></i> Report</button></a>   
                <button type="submit" name="submit" class="btn btn-warning" title="Submit" value="0">Update</button> 
                <button type="submit" name="submit" class="btn btn-success" title="Submit" value="1">Close</button> 
            </div>
...
<script type="text/javascript">
  ...
  function add_joint_on_report(btn, drawing_no, report_number, discipline, module, type_of_module, id_fitup_ex
  	) {
    var url = "<?= site_url('fitup/add_joint_on_report/') ?>" + drawing_no + '/' + report_number + '/' +discipline + '/' +module + '/' +type_of_module + '/' + id_fitup_ex;
    $("#modal").modal({
      show: true,
      keyboard: false,
      backdrop: "static"
