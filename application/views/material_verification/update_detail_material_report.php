    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card border-0 shadow-sm">
          <div class="card-body">
            <h6 class="card-title">Approved For Update Material Detail Report - <strong><?= $main_data['report_number'] ?></strong></h6>
            <hr>
            <form id="form_submition" action="<?= site_url('material_verification/proceed_update_material_verification_report') ?>" method="post">
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="table-responsive overflow-auto">
                  	<div class="text-right <?= $revise_history["status_revise"]!=1 ? 'd-none' : '' ?>">
					          	<span class="btn btn-warning" onclick="add_material_on_report(this, 
					          		'<?= $main_data['drawing_no'] ?>', 
					          		'<?= $main_data['report_number'] ?>',
					          		'<?= $main_data['discipline'] ?>',
												'<?= $main_data['module'] ?>',
												'<?= $main_data['type_of_module'] ?>',
												'<?= $main_data['id_material'] ?>'
					          	)">
                              ...
                              <?php if ($action == "update") : ?>
                                ...
                                <?php if ($value['piping_testing_category'] == 1) : ?>
                                  ...
                                  <?php foreach (explode(";", $value['id_mis_piping']) as $k => $v) : ?>
                                    <input <?= "disabled" ?> type="text" class="form-control" value="<?= $detail_mis[$v]['unique_no'] ?>" onfocus="autocomplete_unique(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>)" onblur="validate_unique_no_2(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>, <?= $k ?>)" required>
                                    <div class="invalid-feedback"></div>
                                    <br>
                                    <input <?= "disabled" ?> type="hidden" name="id_mis_piping_input[<?= $key ?>][]" class="id_mis_pip_input" value="<?= $v ?>">
                                  <?php endforeach; ?>

                                <?php else : ?>
                                  <input <?= "disabled" ?> type="text" class="form-control" value="<?= $detail_mis[$value['id_mis']]['unique_no'] ?>" onfocus="autocomplete_unique(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>)" onblur="validate_unique_no(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>)" required>
                                  <div class="invalid-feedback"></div>
                                <?php endif; ?>
                            ...
                            <td>
                              ...
                              <?php if ($action == "update") : ?>
                                ...
                                <?php if ($value['piping_testing_category'] == 1) : ?>
                                  ...
                                  <?php foreach (explode(";", $value['id_mis_piping']) as $k => $v) : ?>
                                    <input <?= "disabled" ?> type="text" class="form-control heat_no_piping_<?= $k ?>" value="<?= $detail_mis[$v]['heat_or_series_no'] ?>" disabled>
                                    <br>
                                  <?php endforeach; ?>

                                <?php else : ?>
                                  <input <?= "disabled" ?> type="text" class="form-control heat_no" value="<?= $detail_mis[$value['id_mis']]['heat_or_series_no'] ?>" disabled>
                                <?php endif; ?>
                              ...
                              <?php if ($action == "update") : ?>
                                ...
                                <?php if ($value['piping_testing_category'] == 1) : ?>
                                  ...
                                  <?php foreach (explode(";", $value['id_mis_piping']) as $k => $v) : ?>
                                    ...
                                    ?>
                                    <input <?= "disabled" ?> type="text" class="form-control mrir_piping_<?= $k ?>" value="<?= $report_no ?>" disabled>
                                    <br>
                                  <?php endforeach; ?>

                                <?php else : ?>
                                  <input <?= "disabled" ?> type="text" class="form-control mrir" value="<?= $detail_mis[$value['id_mis']]['heat_or_series_no'] ?>" disabled>
                                <?php endif; ?>
                              ...
                              <td class="text-left">
                                <div class="form-check">
                                  <label class="form-check-label">
                                    <input <?= "disabled" ?> type="radio" class="form-check-input radio_button approve" name="approval[<?= $key ?>]" value="3" style="transform: scale(1.3);"><b class="text-success">Approve</b>
                                  </label>
                                </div>
                                <div class="form-check">
                                  <label class="form-check-label">
                                    <input <?= "disabled" ?> type="radio" class="form-check-input radio_button reject" name="approval[<?= $key ?>]" value="2" style="transform: scale(1.3);"><b class="text-danger">Reject</b>
                                  </label>
                                </div>
                                <div class="form-check">
                                  <label class="form-check-label">
                                    <input <?= "disabled" ?> type="radio" class="form-check-input radio_button pending" name="approval[<?= $key ?>]" value="4" style="transform: scale(1.3);"><b class="text-info">Pending By QC</b>
                                  </label>
                                </div>
                              </td>
                              <td>
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="float-right">
                    ...
                    <button type="submit" name="submit" class="btn btn-warning" value="0"><i class="fas fa-edit"></i> Update</button>
                    <button type="submit" name="submit" class="btn btn-success" value="1"><i class="fas fa-check"></i> Close</button>
...
<script>
	...
	<?php if($revise_history["status_revise"]!=1){ ?>
		$('#form_submition :input').prop('disabled', true);
		$('#form_submition :select').prop('disabled', true);
	<?php } ?>

	function add_material_on_report(btn, drawing_no, report_number, discipline, module, type_of_module, id_material_ex
  	) {
    var url = "<?= site_url('material_verification/add_material_on_report/') ?>" + drawing_no + '/' + report_number + '/' +discipline + '/' +module + '/' +type_of_module + '/' + id_material_ex;
    $("#modal").modal({
      show: true,
      keyboard: false,
      backdrop: "static"
    }).find('.modal-body').load(url)
    $('.modal-title').text("Additional Item")
