...
$id_user_enc = strtr($this->encryption->encrypt($this->user_cookie[0]), '+=/', '.-~');
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card border-0 shadow-sm">
          <div class="card-body">
            <h6 class="card-title">Material Inspection Detail - <strong><?= $main_data['submission_id'] ?></strong></h6>
            <hr>
            <div class="row">
              <div class="col-md-6">
            ...
            <hr>
            <form action="<?= site_url('material_verification/proceed_approval_inspection') ?>" method="post">
                      ...
                      <div class="row mt-3">
                              ...
                              <tbody>
                                ...
                                foreach ($detail_data as $key => $value) : ?>
                                  ...
                                  if ($value['as_rev_no'] != '') {
                                    ...
                                    // $drawing_as_rev = isset($shop_drawing[$value['drawing_as']]) ? $shop_drawing[$value['drawing_as']]['last_revision_no'] : (isset($data_drawing[$value['drawing_as']]) && $data_drawing[$value['drawing_as']]['last_revision_no'] == '' ? '01' : $data_drawing[$value['drawing_as']]['last_revision_no']) ;
                                  ...
                                  if ($value['sp_rev_no'] != '') {
                                    ...
                                    // $drawing_sp_rev = isset($shop_drawing[$value['drawing_sp']]) ? $shop_drawing[$value['drawing_sp']]['last_revision_no'] : (isset($data_drawing[$value['drawing_sp']]) && $data_drawing[$value['drawing_sp']]['last_revision_no'] == '' ? '01' : $data_drawing[$value['drawing_sp']]['last_revision_no']);
                                  ...
                                  $link_mrir_det = $wh_link . 'mb/detail_mrir_cs/'.$unique_disc.'/?id=' . $mrir_id_enc . '&action=' . $action_enc . '&status=' . $status_enc . '&partial_report_no=' . $partial_report_no . '&user=' . $id_user_enc;
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                      <?php if ($value['drawing_sp']) : ?>
                                        ...
                                        $latest_sp_rev = $drawing_sp_rev;

                                        ?>
                                        <input type="hidden" name="sp_rev_no[<?= $key ?>]" value="<?= $drawing_sp_rev ?>">

                                        <input type="hidden" name="latest_sp_rev_no[<?= $key ?>]" value="<?= $latest_sp_rev ?>">
                                    ...
                                    <td>
                                      <?php if ($value['drawing_as']) : ?>
                                        ...
                                        $latest_as_rev = $drawing_as_rev;

                                        ?>

                                        <input type="hidden" name="as_rev_no[<?= $key ?>]" value="<?= $drawing_as_rev ?>">

                                        <input type="hidden" name="latest_as_rev_no[<?= $key ?>]" value="<?= $latest_as_rev ?>">
                                    ...
                                    <td class="text-nowrap">
                                      <?php if ($value['piping_testing_category'] == 1) : ?>
                                        ...
                                        <?php foreach (explode(";", $value['id_mis_piping']) as $v) : ?>
                                          <?php 
                                            $unique_no   = $detail_mis[$v]['unique_no'];
                                            $mrir_id_enc = strtr($this->encryption->encrypt($mrir_detail[$unique_no]['mrir_id']), '+=/', '.-~');
                                            $action_enc  = strtr($this->encryption->encrypt("detail"), '+=/', '.-~');
                                            $status_enc  = strtr($this->encryption->encrypt($mrir_detail[$unique_no]['status']), '+=/', '.-~');
                                            $partial_report_no = strtr($this->encryption->encrypt($mrir_detail[$unique_no]['partial_report_no']), '+=/', '.-~');
                                            ...
                                            $report_no  = $report_no[1] . '-' . $detail_mis[$v]['partial_report_no'];
                                             ...
                                             $link_mrir_det = $wh_link . 'mrir/detail_mrir_cs/?id=' . $mrir_id_enc . '&action=' . $action_enc . '&status=' . $status_enc . '&partial_report_no=' . $partial_report_no . '&user=' . $id_user_enc;
                                            ?>
                                          <a href="<?= $link_mrir_det ?>" target="_blank"><?= $report_no ?></a>
                                          <br>
                                        <?php endforeach; ?>
                        ...
                        <div class="col-md-12">
                          ...
                          <div class="float-right">
                            ...
                            <a href="<?= site_url('material_verification/material_verification_pdf/' . $submission_enc . '/' . $rev_inspection_enc) ?>" class="btn btn-danger"><i class="fas fa-file-pdf"></i> Report</a>
                            <?php if ($total_approved == $total_data || $total_data == $total_transmit || $total_data == ($total_approved + $total_transmit) || $total_data == $total_approved_client) : ?>
                              ...
                              <tbody>
                                ...
                                foreach ($attachment_list as $key => $value) : ?>
                                  <tr>
                                    ...
                                    <td><?= $user_redline[$value['upload_by']]['full_name'] ?></td>
...
<script>
  ...
  function request_for_update(btn, submission_id) {
    var url = "<?= site_url('material_verification/request_for_update/') ?>" + submission_id
    $("#modal").modal({
      show: true,
      keyboard: false,
      backdrop: "static"
...
`
  ...
  function update_data(btn) {
    ...
    $.ajax({
      url: "<?= site_url('material_verification/update_data_location') ?>",
      type: "POST",
      data: {
