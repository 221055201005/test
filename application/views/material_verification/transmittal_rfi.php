...
$status_checkbox = $submission_id || ($this->input->post('company_id') == 13 && $drawing_no != '') ? '' : 'disabled';
  ...
  <div class="container-fluid">
              ...
              <div class="card-body">
                <form action="" method="post">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-md-6 <?= $project_id == 21 ? null : 'd-none' ?>" id="div_deck">
                      <div class="form-group row">
                        <label for="" class="col-xl-3 col-form-label text-muted">Deck Elevation / Service Line</label>
                        <div class="col-xl">
                          <select name="deck_elevation" id="deck_change" class="select2" style="width:100%" required>
                            <option value="">---</option>
                            <?php foreach ($deck_elevation_list as $key => $value) : ?>
                              <option value="<?= $value['id'] ?>"  <?= $value['id'] == $this->input->post('deck_elevation') ? 'selected' : ''  ?>><?= $value['name'] ?></option>
                            <?php endforeach; ?>
                          </select>
                        </div>
                      </div>
    ...
    <div class="row mt-3">
      <div class="col-md-12">
        <div class="card shadow rounded-0">
          <div class="card-header">
            <h6 class="m-0 card-title">Transmittal RFI List</h6>
          </div>
          <div class="card-body">
            <hr>
            <form id="form_transmit" action="<?= site_url('material_verification/proceed_transmit_material_verification') ?>" method="post">
              ...
              <input type="hidden" name="company_id_filter" value="<?= $this->input->post('company_id') ?>">
              <?php if ($submission_id || ($this->input->post('company_id') == 13 && $drawing_no != '')) : ?>
                ...
                <div class="row">
                  ...
                  <?php if ($this->input->post('company_id') == 13 && $discipline == 1 && (in_array($this->user_cookie[0], [1000392, 1000367, 1000297, 1001385, 1000369]))) : ?>
                    <div class="col-md-4 mt-2">
                      <div class="form-group row">
                        <label for="" class="col-xl-4 col-form-label text-muted">Manual Report No</label>
                        <div class="col-xl">
                          <input type="number" name="temporary_report_number" id="" class="form-control" placeholder="Type Number Only" required>
                        </div>
                      </div>
                    </div>
                  ...
                  <?php if ($this->user_cookie[0] == '1000127' || $this->user_cookie[0] == '1000297') { ?>
                    <!-- <div class="col-md-12">
                  <hr>
                </div>
                <div class="col-md-4">
                  <div class="form-group row">
                    <label for="" class="col-xl-4 col-form-label text-muted"><strong><i>Temporary Report
                          Number</i></strong></label>
                    <div class="col-xl">
                      <input type="number" name="temporary_report_number" class="form-control"
                        placeholder="Temporary Report Number">
                    </div>
                  </div>
                </div> -->
                  <?php } ?>
                  ...
                  <div class="table-responsive overflow-auto">
                    <table class="table table-hover text-center" id="table_transmit" style="width:100%">
                      <thead class="bg-gray-table text-nowrap">
                        <th>
                          <input type="checkbox" name="" id="" class="check_all" style="width:20px; height:20px" <?= $submission_id || ($this->input->post('company_id') == 13 && $drawing_no != '') ? '' : 'disabled' ?>>
                        </th>
                        <th>Project</th>
                        <!-- <th>Work Pack No</th> -->
...
<script>
  $(document).ready(function() {
    find_module_by_project_post(<?= $project_id ?>)
    function find_module_by_project_post(project) {
    ...
    $("#table_transmit").DataTable({
      ...
      columnDefs: [{
        ...
        render: function(data, row, arr) {
          ...
          return `<input type="checkbox" class="check" name="id[${arr[23]}]" value="${data}" style="width:20px; height:20px;" ${status_checkbox} ${is_checked}>`
        }
      }],
    ...
    $(".submission_number").select2({
      ...
      ajax: {
        url: "<?= site_url('material_verification/list_approved_submission_id') ?>",
        dataType: "json",
        type: "GET",
