		...
		<div id="content" class="container-fluid">
			<div>
				...
				<table id="table_mdb" style="border-collapse: collapse;" width="100%" border="1" class="">
					<thead>
						<tr>
							<th colspan="6">MDB GENERAL – GENERAL FABRICATION PROCEDURE</th>
						</tr>
						<tr>
							<th width='1%'>S/N</th>
									...
									<tbody class="cat_<?= $section['var_code'] ?>" style="display: none;">
										...
										<?php foreach ($file_list[$section['var_code']] ?? [] as $key => $file) : ?>
											<tr>
												<td></td>
												<td></td>
												<td><a href="<?= $file['link'] ?>" target="_blank"><?= $section['volume'] ?>.<?= $section['section'] ?>.<?= $key + 1 ?> <?= $file['document_no'] ?></a></td>
												<td><?= $file['ecodoc_no'] ?></td>
										...
										<tbody class="cat_<?= $subsection['var_code'] ?>" style="display: none;">
											...
											<?php foreach ($file_list[$subsection['var_code']] ?? [] as $key => $file) : ?>
												<tr>
													<td></td>
													<td></td>
													<td><a href="<?= $file['link'] ?>" target="_blank"><?= $subsection['volume'] ?>.<?= $subsection['section'] ?>.<?= $subsection['subsection'] ?>.<?= $key + 1 ?> <?= $file['document_no'] ?></a></td>
