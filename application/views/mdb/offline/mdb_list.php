<?php
$value_deck = $deck_elevation[0];
// test_var($mdb_general_volume_list);
?>
<!doctype html>
<html lang="en">
        ...
        <tbody class="cat_<?= $section['var_code'] ?>" style="display: none;">
          ...
          <?php foreach ($file_list[$section['var_code']] ?? [] as $key => $file) : ?>
            <tr>
              ...
              <td></td>
              <td>
                <a href="<?= $params['link_' . $section['var_code']] . '/' . $file['att_link'] ?>" target="_blank"><?= $section['volume'] ?>.<?= $section['section'] ?>.<?= $key + 1 ?> <?= $file['report_number'] ?></a>
              </td>
