      ...
      <div class="col-md-12">
        <div class="card border-0 shadow-sm">
          <div class="card-body">
            ...
            <hr>
            <form
              action="<?= $action == "update" ? site_url('itr/proceed_update_itr') : site_url('itr/proceed_re_approval_itr') ?>"
              method="post">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <input type="text" class="form-control" value="<?= $discipline_name[$main_data['discipline']] ?>"
                        disabled>
                    </div>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <input type="text" class="form-control" value="<?= $project_name[$main_data['project_code']] ?>"
                        disabled>
                    </div>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <input type="text" class="form-control" value="<?= $module_name[$main_data['module']] ?>"
                        disabled>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <input type="text" class="form-control" value="<?= $type_module[$main_data['type_of_module']] ?>"
                        disabled>
                    </div>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <select name="area_v2" style="width:100%" onchange="get_location_list(this)"  class="select2"
                        <?= $action == "update" ? 'required' : 'disabled' ?>>
                        <option value="">---</option>
                        <?php foreach ($area_v2 as $key => $value): ?>
                        <option value="<?= $value['id'] ?>"
                          <?= $value['id'] == $main_data['area_v2'] ? 'selected' : '' ?>>
                          <?= $value['name'] ?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-xl">
                      <select name="location_v2" onchange="get_point_list(this)" class="select2" <?= $action == "update" ? 'required' : 'disabled' ?>>
                        ...
                        <option value="<?= $value['id'] ?>"
                          <?= $value['id'] == $main_data['location_v2'] ? 'selected' : '' ?>><?= $value['name'] ?>
                        </option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                ...
                <div class="col-md-6">
                  <div class="form-group row">
                    <label class="col-xl-3 col-form-label text-muted" for="">Point <span
                        style="font-size: 10px;"><strong><i>(Optional)</i></strong></span></label>
                    <div class="col-xl">
                      <select name="point_v2" class="select2" <?= $action == "update" ? '' : 'disabled' ?>>
                        <option value="0">---</option>
                        <?php foreach ($point_list as $key => $value): ?>
                        <option value="<?= $value['id'] ?>"
                          <?= $value['id'] == $main_data['point_v2'] ? 'selected' : '' ?>><?= $value['name'] ?>
                        </option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                ...
                <div class="col-md-12">
                  <div class="table-responsive overflow-auto">
                    <table class="table table-hover text-center text-nowrap">
                      ...
                      <tbody>
                        <?php $no = 1; foreach ($detail_material as $key => $value): ?>
                        <?php 
                        $report_no  = explode('/', $detail_mis[$value['id_mis']]['report_no'])   ;
                        $report_no  = $report_no[1];  
                        ?>
                        <tr>
                          <input type="hidden" name="id_mis[<?= $key ?>]" value="<?= $value['id_mis'] ?>"
                            class="id_mis">
                          ...
                          <td>
                            ...
                            <input type="text" class="form-control"
                              value="<?= $detail_mis[$value['id_mis']]['unique_no'] ?>"
                              onfocus="autocomplete_unique(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>)"
                              onblur="validate_unique_no(this, '<?= $workpack['workpack_no'] ?>', <?= $value['grade'] ?>, <?= $value['id_workpack'] ?>)"
                              required>
                          ...
                          <td>
                            <?php if ($action == "update"): ?>
                            <input type="text" class="form-control heat_no"
                              value="<?= $detail_mis[$value['id_mis']]['heat_or_series_no'] ?>" disabled>
                            <?php else: ?>
                          ...
                          <td>
                            <?php 
                              ...
                              $list_wps_id = explode(";", $value['wps_id']);
                            
                            ?>
                            <select name="wps_id[<?= $key ?>][]"  class="select2" required multiple>
                              <option value="">---</option>
                              <?php foreach ($wps_list as $v): ?> 
                          ...
                          <td>
                            <?php 
                              ...
                              $list_welder_id = explode(";", $value['welder_id']);
                            
                            ?>
                            <select name="welder_id[<?= $key ?>][]"  class="select2" required multiple>
                              <option value="">---</option>
                              <?php foreach ($welder_list as $v): ?> 
                          ...
                          <td class="text-left">
                            <div class="form-check">
                              <label class="form-check-label">
                                <input type="radio" class="form-check-input radio_button approve"
                                  name="approval[<?= $key ?>]" value="3" style="transform: scale(1.3);"><b
                                  class="text-success">Approve</b>
                              </label>
                            </div>
                            <div class="form-check">
                              <label class="form-check-label">
                                <input type="radio" class="form-check-input radio_button reject"
                                  name="approval[<?= $key ?>]" value="2" style="transform: scale(1.3);"><b
                                  class="text-danger">Reject</b>
                              </label>
                            </div>
                            <div class="form-check">
                              <label class="form-check-label">
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="float-right">
                    <a href="<?= site_url('material_verification/request_for_update_list/'.strtr($this->encryption->encrypt($action == "update" ? 'approved_for_update' : 're_approval'), '+=/', '.-~'));?>"
