    <footer class="container-fluid bg-white py-2">
      <div class="row">
        <div class="col-12 col-md">
          <center><small class="d-block my-1 text-muted">&copy; 2021 - SEATRIUM - PROJECT CONTROL MANAGEMENT SYSTEM 2.1</small></center>
        </div>
      </div>
    </footer>
      ...
      function sweeralert_confirm(type, text, input = null, e = null, func_name = null, remarks = '') {
        if($(input).is("[href]")){
          ...
          if($(input).prop("type") == "submit"){
						const form_input = $(input).closest("form")[0];
						if (form_input.checkValidity()) {
        			$(form_input).submit()
						} else {
							form_input.reportValidity();
						}
