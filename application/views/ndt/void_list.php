          ...
          <div class="overflow-auto">
            <table class="table table-hover text-center dataTable" width="100%">
              ...
              <tbody>
                ...
                <tr>
                  <td><?= $this->master_rfi_no[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['ndt_rfi'.($value['id_vendor']==13 ? '_13' : '')].'<strong>'.strtoupper($ndt[$value['ndt_type']]['ndt_initial']).'-'.str_pad($value['visual_transmittal_no'], 4, 0, STR_PAD_LEFT ).'</strong>' ?></td>
  ...
  function approvalDialog(id, status_approval){
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.value==true) {
        $.ajax({
          url: "<?= base_url('ndt/void_approval_v2/') ?>",
          type: "post",
          data: {
            'id': id,
