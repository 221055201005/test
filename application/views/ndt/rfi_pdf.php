...
<html><head>
    ...
    <table width="100%" border="1px" style="border-collapse: collapse !important;">
      <head>
        <tr>
          ...
          <td><b class="tab2">RFI NDT NO. <?= strtoupper($ndt[$list[0]['ndt_type']]['ndt_initial']) ?></b>: <?= $rfi_no ?></td>
        </tr>
        <tr>
        ...
        <tr>
          <?php //test_var($master_drawing) ?>
          <td><b class="tab">DRAWING NO.</b>: <?= $list[0]['drawing_no'].' Rev. '.($list[0]['status_inspection']==7 || $list[0]['status_inspection']==9 ? $list[0]['drawing_rev_no'] : $master_drawing[$list[0]['drawing_no']]['last_revision_no']).($master_drawing[$list[0]['drawing_no']]['client_doc_no']!='' ? ' ('.$master_drawing[$list[0]['drawing_no']]['client_doc_no'].')' : '') ?></td>
  ...
  <table width="100%" border="0" style="text-align: left;border-collapse: collapse !important;">
    ...
    <tr>
      ...
      <td class="ball" style="vertical-align: middle; text-align: center">
        <?php
          if($value['ndt_type']==3 || $value['ndt_type']==1){
            if($value['revision']>0){
              print_r((int)$value['length_of_weld']);
            } else {
              print_r((int)$joint_detail[$value['joint_id']]['weld_length']);
            }
          } else {
            print_r((int)$joint_detail[$value['joint_id']]['weld_length']);
          }
