          ...
          <form method="POST" action="<?php echo base_url() ?>ndt/send_to_vendor">
            ...
            <div class="overflow-auto">
              <table class="table table-hover text-center dataTable" width="100%">
                <thead class="bg-green-smoe text-white table-bordered">
                  <tr>
                    <th rowspan="2"></th>
                ...
                <tbody>
                  ...
                  <tr <?= $submitted=='submitted' ? (strtotime(DATE('Y-m-d H:i:s'))-strtotime($value['ndt_transmittal_datetime'])>43200 ? 'class=""' : '') : '' ?> >
                    ...
                    <td class="align-middle">
                      <?= $value['ndt_mt']=='1' ? '<i class="fas fa-check"></i>' : '-' ?><br>
                      <b>
                        <?php
                          if($ndt_submit[2][$value['id_visual']]){
                            foreach ($ndt_submit[2][$value['id_visual']] as $keyf => $valuef) { ?>
                              <a href="<?= base_url('ndt/ndt_rfi_report/2/').$valuef.'/'.$value['discipline'].'/'.$value['type_of_module'].'/'.$value['project_code'].'/'.$value['company_id'] ?>">MT-<?= str_pad($valuef, 4, 0, STR_PAD_LEFT) ?></a><hr style="padding: 0px !important;margin: 0px !important;">
                        <?php 
                            }
                          } else {
                            echo "";
                    ...
                    <td class="align-middle">
                      <?= $value['ndt_ut']=='1' ? '<i class="fas fa-check"></i>' : '-' ?><br>
                      <b>
                        <?php
                          if($ndt_submit[3][$value['id_visual']]){
                            foreach ($ndt_submit[3][$value['id_visual']] as $keyf => $valuef) { ?>
                              <a href="<?= base_url('ndt/ndt_rfi_report/3/').$valuef.'/'.$value['discipline'].'/'.$value['type_of_module'].'/'.$value['project_code'].'/'.$value['company_id'] ?>">UT-<?= str_pad($valuef, 4, 0, STR_PAD_LEFT) ?></a><hr style="padding: 0px !important;margin: 0px !important;">
                        <?php 
                            }
                          } else {
                            echo "";
                    ...
                    <td class="align-middle">
                      <?= $value['ndt_rt']=='1' ? '<i class="fas fa-check"></i>' : '-' ?><br>
                      <b>
                        <?php
                          if($ndt_submit[1][$value['id_visual']]){
                            foreach ($ndt_submit[1][$value['id_visual']] as $keyf => $valuef) { ?>
                              <a href="<?= base_url('ndt/ndt_rfi_report/1/').$valuef.'/'.$value['discipline'].'/'.$value['type_of_module'].'/'.$value['project_code'].'/'.$value['company_id'] ?>">RT-<?= str_pad($valuef, 4, 0, STR_PAD_LEFT) ?></a><hr style="padding: 0px !important;margin: 0px !important;">
                        <?php 
                            }
                          } else {
                            echo "";
                    ...
                    <td class="align-middle">
                      <?= $value['ndt_pt']=='1' ? '<i class="fas fa-check"></i>' : '-' ?><br>
                      <b>
                        <?php
                          if($ndt_submit[7][$value['id_visual']]){
                            foreach ($ndt_submit[7][$value['id_visual']] as $keyf => $valuef) { ?>
                              <a href="<?= base_url('ndt/ndt_rfi_report/7/').$valuef.'/'.$value['discipline'].'/'.$value['type_of_module'].'/'.$value['project_code'].'/'.$value['company_id'] ?>">PT-<?= str_pad($valuef, 4, 0, STR_PAD_LEFT) ?></a><hr style="padding: 0px !important;margin: 0px !important;">
                        <?php 
                            }
                          } else {
                            echo "";
