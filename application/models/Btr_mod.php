<?php 
  ...
  class Btr_mod extends CI_Model {

    var $column_order_drawing_list   = array('project','drawing_no','drawing_type','test_pack_no','discipline','module','type_of_module','drawing_no');
    var $column_search_drawing_list  = array('drawing_no','test_pack_no');
    var $order_drawing_list          = array('drawing_no' => 'DESC');
    ...
    private function _query_serverside_drawing_list($where = null){
      ...
      $this->db->select('project, drawing_no, drawing_type,test_pack_no, discipline, module, type_of_module');
      $this->db->from('pcms_joint');
      $this->db->group_by('project, drawing_no, drawing_type,test_pack_no, discipline, module, type_of_module');
    }

    public function btr_signed_list($where = null, $order_by = null) {
      ...
      $this->db->from('pcms_btr_signed');
      $query = $this->db->get(); 
      return $query->result_array();

    }

    public function insert_btr_signed($form_data) {
      $this->db->insert('pcms_btr_signed', $form_data);
    }

    public function update_btr_signed($form_data, $where = null) {
      if(isset($where)) {
        $this->db->where($where);
      }

      $this->db->update('pcms_btr_signed', $form_data);
    }

    public function btr_signed_join($where = null, $order_by = null) {
      if(isset($where)) {
        $this->db->where($where);
      }
      ...
      $this->db->from('pcms_btr_signed a');
      $this->db->join('pcms_joint b','b.id = a.id_joint');
      $query = $this->db->get(); 
      return $query->result_array();
    }

    public function btr_approval_list($where = null, $order_by = null) {
