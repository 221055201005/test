...
class Mc_punch_mod extends CI_Model {
	...
	function mc_punch_list($where = null, $limit = null){
    if(isset($where)){
			$this->db->where($where);
		}
		if(isset($limit)){
		...
		$query = $this->db->get('pcms_mc_punch');
		return $query->result_array();
	}

	public function mc_punch_edit_process_db($data, $where) {
    $data = convert2null($data);
    $this->db->where($where);
    $this->db->update("pcms_mc_punch", $data);
  }

  public function mc_punch_import_process_db($data) {
		$data = convert2null($data);
    $this->db->insert_batch('pcms_mc_punch', $data);
  }

	public function mc_punch_list_datatable_db($cat, $where = NULL){
		$table = 'pcms_mc_punch';
		$column_order = array('id', 'project', 'module', 'type_of_module', 'discipline', 'punch_id_no', 'event_id_no', 'system', 'subsystem', 'checklist', 'tag_no', 'description', 'cat', 'action_by', 'phase', 'last_update_date', 'status', 'status', 'id');
		$column_search = array('punch_id_no', 'event_id_no', 'system', 'subsystem', 'checklist', 'tag_no', 'description', 'cat', 'action_by', 'phase', 'last_update_date');
		$column_select = "id, project, module, type_of_module, discipline, punch_id_no, event_id_no, system, subsystem, checklist, tag_no, description, cat, action_by, phase, last_update_date, status, status, id";
		...
		foreach ($column_search as $key => $item){
			if ($_POST['search']['value']){
				if ($i === 0){
					...
					$this->db->like('CAST('.$column_search[$key].' AS VARCHAR)', $_POST['search']['value']);
				}
				else{
					$this->db->or_like('CAST('.$column_search[$key].' AS VARCHAR)', $_POST['search']['value']);
				}
	...
	function mc_punch_document_register_list($where = null, $limit = null){
    if(isset($where)){
			$this->db->where($where);
		}
		if(isset($limit)){
		...
		$query = $this->db->get('pcms_mc_punch_document_register');
		return $query->result_array();
	}

  public function mc_punch_document_register_new_process_db($data) {
    $this->db->insert('pcms_mc_punch_document_register', $data);
		$insert_id = $this->db->insert_id();
		return $insert_id;
  }

	public function mc_punch_document_register_edit_process_db($data, $where) {
    $this->db->where($where);
    $this->db->update("pcms_mc_punch_document_register", $data);
  }

	function mc_punch_attachment_list($where = null, $limit = null){
    if(isset($where)){
			$this->db->where($where);
		}
		if(isset($limit)){
		...
		$query = $this->db->order_by("created_date", "DESC");
		$query = $this->db->get('pcms_mc_punch_attachment');
		return $query->result_array();
	}

  public function mc_punch_attachment_new_process_db($data) {
    $this->db->insert('pcms_mc_punch_attachment', $data);
		$insert_id = $this->db->insert_id();
		return $insert_id;
  }

	public function mc_punch_attachment_edit_process_db($data, $where) {
    $this->db->where($where);
    $this->db->update("pcms_mc_punch_attachment", $data);
  }

  function mc_punch_rfi_list($where = null, $limit = null){
    if(isset($where)){
			$this->db->where($where);
		}
		if(isset($limit)){
		...
		$query = $this->db->get('pcms_mc_punch_rfi');
		return $query->result_array();
	}

  public function mc_punch_rfi_new_process_db($data) {
    $this->db->insert('pcms_mc_punch_rfi', $data);
		$insert_id = $this->db->insert_id();
		return $insert_id;
  }

	public function mc_punch_rfi_edit_process_db($data, $where) {
    $this->db->where($where);
    $this->db->update("pcms_mc_punch_rfi", $data);
  }

  function mc_punch_rfi_attachment_list($where = null, $limit = null){
    if(isset($where)){
			$this->db->where($where);
		}
		if(isset($limit)){
		...
		$query = $this->db->order_by("created_date", "DESC");
		$query = $this->db->get('pcms_mc_punch_rfi_attachment');
		return $query->result_array();
	}

  public function mc_punch_rfi_attachment_new_process_db($data) {
    $this->db->insert('pcms_mc_punch_rfi_attachment', $data);
		$insert_id = $this->db->insert_id();
		return $insert_id;
  }

	public function mc_punch_rfi_attachment_edit_process_db($data, $where) {
    $this->db->where($where);
    $this->db->update("pcms_mc_punch_rfi_attachment", $data);
  }
  ...
  public function load_data_dashboard($where = null) {
    ...
    $this->db->select('
      ...
      COUNT(CASE WHEN status = 2 THEN 1 END) AS total_completed_work,
      COUNT(CASE WHEN status = 3 THEN 1 END) AS total_pending,
      COUNT(CASE WHEN status = 4 THEN 1 END) AS total_rejected,
      COUNT(CASE WHEN status = 5 THEN 1 END) AS total_approved
    ');

    $this->db->from('pcms_mc_punch');
    $query = $this->db->get(); 
    return $query->result_array();
  }
