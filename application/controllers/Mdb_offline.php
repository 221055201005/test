...
class Mdb_offline extends CI_Controller {
  ...
  protected function generate_index_deck($data, $params) {
    error_reporting(0);
    $index_name                   = $params['index_loc'].'/'.$params['index_name'].'.html';
  ...
  public function wtr_dossier($deck, $action = "view", $file_loc = null) {
    ...
    foreach($datadb as $value) {
      ...
      $att_name            = $value['drawing_no'].'.html';
      if (file_exists($file_loc.'/'.$att_name)) {
        $output[]               = 1;
        continue;
      }
      ...
      if(@!in_array($value['project'], $params['project'])) {
        $params['project'][]        = $value['project'];
      }
      if(@!in_array($value['drawing_no'], $params['drawing_no'])) {
        $params['drawing_no'][]     = $value['drawing_no'];
      }
      if(@!in_array($value['drawing_type'], $params['drawing_type'])) {
        $params['drawing_type'][]   = $value['drawing_type'];
      }
      if(@!in_array($value['discipline'], $params['discipline'])) {
        $params['discipline'][]     = $value['discipline'];
      }
      if(@!in_array($value['module'], $params['module'])) {
        $params['module'][]         = $value['module'];
      }
      if(@!in_array($value['type_of_module'], $params['type_of_module'])) {
        $params['type_of_module'][] = $value['type_of_module'];
      }
      $keys                       = $value['project'].'#'.$value['drawing_no'].'#'.$value['drawing_type'].'#'.$value['discipline'].'#'.$value['module'].'#'.$value['type_of_module'];
      if(@!in_array($keys, $params['keys'])) {
        $params['keys'][]           = $keys;
      }
    ...
    if($action == "save" && $params) {
      $this->wtr_html_new($params, $file_loc);
    }
  ...
  public function wtr_html_new($params, $offline_loc = null){
    error_reporting(0);
		$pdf = null;
		...
		$datadb = $this->fitup_mod->wps_code_version2();
		foreach ($datadb as $key => $value) {
    ...
    $where_ndt[implode_where("b.project_code", $params['project'])] = null;
    $where_ndt[implode_where("b.drawing_no", $params['drawing_no'])] = null;
    $where_ndt[implode_where("b.discipline", $params['discipline'])] = null;
    $where_ndt[implode_where("b.module", $params['module'])] = null;
    $where_ndt[implode_where("b.type_of_module", $params['type_of_module'])] = null;
		$where_ndt["a.result IN (2,3) "]   	= null;

		$ndt = $this->wtr_mod->ndt_list_data_m($where_ndt);
		foreach ($ndt as $key => $value) {
			$data['ndt'][$value['id_joint_visual']][$value['ndt_type']] = $value;  			 
			$data['ndt_all'][$value['id_joint_visual']][$value['ndt_type']][$value["revision_category"]][$value["revision"]][] = $value;			
		}
		unset($where_ndt);
		
		// $where["a.project"] 		= $project;	
		// $where["a.discipline"]   	= $drawing_no;		
		// $where["a.discipline"]  	= $discipline;	
		// $where["a.module"]   	   	= $module;		
    // $where["a.type_of_module"]  = $type_of_module;

    $where[implode_where("a.project", $params['project'])] = null;
    $where[implode_where("a.drawing_no", $params['drawing_no'])] = null;
    $where[implode_where("a.discipline", $params['discipline'])] = null;
    $where[implode_where("a.module", $params['module'])] = null;
    $where[implode_where("a.type_of_module", $params['type_of_module'])] = null;

		$where["(b.status_retransmitted = 0 OR b.status_retransmitted IS NULL) AND b.status_resubmit <> 1"]   	= NULL;
		$where["(c.retransmitt_status = 0 OR c.retransmitt_status IS NULL) AND c.status_delete IS NULL"]   	= NULL;
		// $data['wtr_list']  = $this->wtr_mod->wtr_all_of_joint_list($where); 
		$wtr_list   = $this->wtr_mod->wtr_all_of_joint_list($where); 
    unset($where);
    
    $piecemark_list_submit_1 = array(); 
    $piecemark_list_submit_2 = array(); 
    
    foreach($wtr_list as $value) {
      $keys                       = $value['project'].'#'.$value['drawing_no'].'#'.$value['drawing_type'].'#'.$value['discipline'].'#'.$value['module'].'#'.$value['type_of_module'];
      $wtr[$keys][]               = $value;

      $list_fitter_id[] = $value['fitter_id'];
      $list_welder_ref_rh[] = $value['welder_ref_rh'];
      $list_welder_ref_fc[] = $value['welder_ref_fc'];
      $list_drawing_no[]   = $value['drawing_no'];
      $list_drawing_wm[]   = $value['drawing_wm'];
      $piecemark_list_submit_1[] =  $value['pos_1']; 
			$piecemark_list_submit_2[] =  $value['pos_2']; 
    }

		// Fitter Code
			$fitter_id_s = $list_fitter_id;
			foreach ($fitter_id_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$fitter_id[] = intval($values);
				}
			}
			if($fitter_id){
				$datadb = $this->wtr_mod->fitter_code(["id_fitter IN (".implode(', ', $fitter_id).")" => NULL]);
				foreach ($datadb as $key => $value) {
					$data['fitter_code_arr'][$value['id_fitter']] = $value['fit_up_badge'];
					$data['master_fitter'][$value['id_fitter']] = $value;
				}
			}

		// Welder Code
			$welder_ref_rh_s = $list_welder_ref_rh;
			foreach ($welder_ref_rh_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$welder_ref[] = $values;
				}
			}
			$welder_ref_fc_s = $list_welder_ref_fc;
			foreach ($welder_ref_fc_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$welder_ref[] = $values;
				}
			}
			foreach ($welder_ref as $key => $value) {
				if($value!=0){
					$id_welder[] = $value;
				}
			}
			if($id_welder){
				$datadb = $this->fitup_mod->welder_code_version2(["a.id_welder IN (".implode(', ', $id_welder).")" => NULL]);
				foreach ($datadb as $key => $value) {
					$data['master_welder'][$value['id_welder']] = $value;
				}
			}
 
		$drawing_nox   	= $list_drawing_no; 
		$drawing_no_wmx  = $list_drawing_wm; 
		...
		if(sizeof(array_filter(array_column($datadb, 'id_mis'))) > 0){
			...
			$datadb = $this->wtr_mod->warehouse_mis_mrir($where);
			unset($where);
			foreach ($datadb as $key => $value) {
    ...
    $data['user_cookie'] 	 = $this->user_cookie;
		...
		$data['meta_title']  	 = 'MWTR Drawing List';

    foreach($params['keys'] as $value) {
      $data['wtr_list']         = $wtr[$value];
      $data['joint_list']       = $data['wtr_list'];

      $explode_val              = explode("#", $value);
      $data["project"] 		      = $explode_val[0];	
      $data["drawing_no"]   	  = $explode_val[1];		
      $data["drawing_type"]     = $explode_val[2];
      $data["discipline"]  	    = $explode_val[3];	
      $data["module"]   	   	  = $explode_val[4];		
      $data["type_of_module"]   = $explode_val[5];		
      $html_name	= $offline_loc.'/'.$explode_val[1].'.html';

      $myfile   	= fopen($html_name, "w");
  
      // $view     	= $this->load->view('irn/irn_pdf_joint_landscape', $data, true);
      $view 			= $this->load->view('wtr/wtr_list_detail_pdf_3', $data, true);  
   ...
   public function ht_dossier($deck, $action = "view", $offline_loc = null, $sftp = null){
    ...
    foreach ($datadb as $key => $value) {
      ...
      $att_name            = $value['report_number'].'.pdf';
      if (file_exists($offline_loc.'/'.$att_name)) {
        $output[$value['type_of_module']][] = 1;
        continue;
      }

      if(@count($output[$value['type_of_module']]) > $this->limit) {
        continue;
      }
  ...
  public function irn_dossier($deck, $action = "view", $offline_loc = null){
    ...
    $params = [];
      ...
      if(@!in_array($value['submission_id'], $params['submission_id'])) {
        $params['submission_id'][]  = $value['submission_id'];
      }

      if(@!in_array($value['report_number'], $params['report_number'])) {
        $params['report_number'][]  = $report_no;
      }
      
      $keys               = $value['submission_id'].'#'.$report_no;
      if(@!in_array($keys, $params['keys'])) {
        $params['keys'][]  = $keys;
      }
    ...
    if($action == "save" && $params) {
      $this->irn_pdf_new($params, $offline_loc);
    }
    
		return $output;
  }
  
  public function irn_pdf_new($params,$file_loc = null){
    error_reporting(0);
    ...
    $where[implode_where("a.submission_id", $params['submission_id'])] = null;
    $where['b.status_delete'] = 1;
    $show_pcms_irn_all 				= $this->irn_mod->show_pcms_irn($where);
    unset($where['b.status_delete']);

    $drawing_unique_irn = [];
    foreach($show_pcms_irn_all as $value) {
      // PASS 1
      $show_pcms_irn[$value['submission_id']][] = $value;
      $drawing_unique_list[]  = $value['drawing_no'];
      if(@!in_array($value['drawing_no'], $drawing_unique_irn[$value['submission_id']])) {
        // PASS 2
        $drawing_unique_irn[$value['submission_id']][] = $value['drawing_no'];
      }

      $list_id_user[]         = intval($value['smoe_approval_by']);
      $list_id_user[]         = intval($value['client_approval_by']);
    }

    $show_pcms_irn_description_all 	= $this->irn_mod->show_pcms_irn_description($where);		
    foreach($show_pcms_irn_description_all as $value) {
      // PASS 3
      $show_pcms_irn_description[$value['submission_id']][] = $value;
    }

    $show_pcms_irn_punchlist_all 	= $this->irn_mod->show_pcms_irn_punchlist($where); 
    foreach($show_pcms_irn_punchlist_all as $value) {
      // PASS 4
      $show_pcms_irn_punchlist[$value['submission_id']][] = $value;
    }

    $show_pcms_irn_detail_all 		= $this->irn_mod->show_pcms_irn_detail($where);  
    foreach($show_pcms_irn_detail_all as $value) {
      // PASS 5
      $show_pcms_irn_detail[$value['submission_id']][]  = $value;

      // PASS 6
      $irn_pcms_detail[$value['submission_id']][$value['id_master_irn_detail']]  = $value;
    }

    $show_pcms_irn_dc_all 			= $this->irn_mod->show_irn_dc($where); 
    foreach($show_pcms_irn_dc_all as $value) {
      // PASS 7
      $show_pcms_irn_dc[$value['submission_id']][] =  $value;
      $id_detail_dimension_list[]   = $value['id_detail_dimension'];
    }
    ...
    $where[implode_where("submission_id", $params['submission_id'])]  = null;
    ...
    foreach($validated_data as $key => $value){
	  ...
	  $drawing_unique = array_unique($drawing_unique_list);   
	  $where["document_no IN ('".implode("', '", $drawing_unique)."')"] = NULL;   
	  $where["status_delete"] = 1;   
	  $datadb = $this->wtr_mod->data_drawing_list($where);
	  unset($where);
	  if (sizeof($datadb) > 0) {
    ...
    $data["project_data_portal"]     = $this->general_mod->read_project_name(12);

    if(sizeof($id_detail_dimension_list) > 0){
      $pos_merge = array_unique($id_detail_dimension_list); 
    ...
    $where[implode_where("e.submission_id", $params['submission_id'])]  = null;
    $result_db =  $this->irn_mod->show_data_irn_joint($where);
    unset($where); 
    
    foreach($result_db as $value) {
      // PASS 8
      $result[$value['submission_id_irn']][]  = $value;
      $id_joint_list[]  = $value['id'];
      $pos_1_list[]     = $value['pos_1'];
      $pos_2_list[]     = $value['pos_2'];
    }

    $where["a.joint_id IN ('".implode("', '", $id_joint_list)."')"] = NULL; 
    ...
    $id_user_list             = array_filter(array_unique($list_id_user));
    ...
    foreach($params['keys'] as $value) {
      ...
      $data['show_pcms_irn']                = $show_pcms_irn[$explode_val[0]];
      $data['drawing_unique']               = $drawing_unique_irn[$explode_val[0]];
      $data['show_pcms_irn_description']    = $show_pcms_irn_description[$explode_val[0]];
      $data['show_pcms_irn_punchlist']      = $show_pcms_irn_punchlist[$explode_val[0]];
      $data['show_pcms_irn_detail']         = $show_pcms_irn_detail[$explode_val[0]];
      $data['irn_pcms_detail']              = $irn_pcms_detail[$explode_val[0]];
      $data['show_pcms_irn_dc']             = $show_pcms_irn_dc[$explode_val[0]];

      $data["show_data_irn_list"]           = $result[$explode_val[0]]; 
      $file_name                            = $explode_val[1].'.pdf';

      if($data['drawing_unique']) {
        $wtr_html_loc = $file_loc.'/'.$explode_val[1];
        if(!is_dir($wtr_html_loc)) {
          mkdir($wtr_html_loc, 0777, true);
        }
  
        $params_html        = [];
        foreach($data['drawing_unique'] as $value) {
          if(file_exists($wtr_html_loc.'/'.$value.'.html')) {
            continue;
          }
          $params_html['drawing_no'][]  = $value;
        }
        
        if($params_html) {
          $this->irn_wtr_html_new(encrypt($explode_val[0]), $params_html, $wtr_html_loc, null);
        }
      }
      $data['meta_title'] 	 = "IRN Inspection"; 
      $data['report_format']  = $explode_val[1];
      $data['file_loc']       = $file_loc;
      $data['meta_title'] 	  = "IRN Inspection"; 
      $paper                  = 'A4'; 
      $orientation            = "portrait"; 
      $html = $this->load->view('mdb/offline/irn_pdf',$data, true);	
    ...
    $output = [];
      ...
      $att_name            = $value['report_number'].'.pdf';
      if (file_exists($offline_loc.'/'.$att_name)) {
        $output[$value['type_of_module']][$value['ndt_type']][] = 1;
        continue;
      }

      if(@count($output[$value['type_of_module']][$value['ndt_type']]) > $this->limit) {
        continue;
      }
    ...
    if($action == "save") {
      ...
      $att_name           = $value['document_no'].'.pdf';
      if (file_exists($offline_loc.'/'.$att_name)) {
        $output[$drawing_type][] = 1;
        continue;
      }

      if(@count($output[$drawing_type]) > $this->limit) {
        continue;
      }
	...
	public function shopdrawing_dossier_jacket($action = "view", $offline_loc = null, $dr_type = null, $sftp = null, $desc_assy = NULL){
    ...
    if($desc_assy){
      $where["desc_assy IN (".implode(", ", array_filter($desc_assy)).")"] = NULL;
    }
    ...
    $output                           = [];
      ...
      $this->ecodoc_no_list[] = $value['client_doc_no'];
			...
			if($action == "save" AND !file_exists($offline_loc."/".$value['document_no'].'.pdf')){
        if(isset($dr_list[$value['id']])) {
          $local_file       = $offline_loc."/".$value['document_no'].'.pdf';
          $remote_file      = "/PCMS/pcms_ori/upload/activity_revision/".$dr_list[$value['id']]['attachment'];
          $sftp->get($remote_file, $local_file);
        }
      } elseif($action == "save" AND file_exists($offline_loc."/".$value['document_no'].'.pdf')){
        if(isset($dr_list[$value['id']])) {
          test_var('skiped file '.$value['document_no'].'.pdf'.' already existed!', 1);
  ...
  public function mv_dossier_deck($deck, $action = "view", $offline_loc = null) {
    ...
    if($data_list) {
      ...
      foreach($data_list as $key => $value) {
        ...
        // if($action == "save") {
        //   $this->mv_pdf(encrypt($value['project_code']),encrypt($value['discipline']),encrypt($value['type_of_module']),encrypt($value['module']),encrypt($value['report_number']),encrypt($value['report_no_rev']),encrypt($value['drawing_no']), encrypt($offline_loc));
        // }

        $arr_transmittal_date[] = $value['transmittal_datetime'];
        $arr_report_number[]    = $value['report_number'];
        $arr_combined[]         = $value['transmittal_datetime'].'_'.$value['report_number'];

        
      }

      $key_report_list          = [
        'transmittal_datetime'  => $arr_transmittal_date,
        'report_number'         => $arr_report_number,
        'combined'              => $arr_combined
      ];

      if($action == "save") {
        if($arr_transmittal_date) {
          $this->mv_pdf_new($key_report_list, $offline_loc);
        }
      }
    }
  ...
  protected function mv_pdf_new($key_list, $file_loc = null) {
    error_reporting(0);

    $where[implode_where("transmittal_datetime", $key_list['transmittal_datetime'])] = null;
    $where[implode_where("report_number", $key_list['report_number'])] = null;
    ...
    foreach($detail as $value) {
      $combined_key           = $value['transmittal_datetime'].'_'.$value['report_number'];

      $detail_mv[$combined_key][]  = $value;
      if(in_array($value['status_inspection'], [3,5,7])) {
        @$total_approved[$combined_key] += 1;
      }

      if($value['status_inspection'] == 7) {
        @$total_approved_client[$combined_key] += 1;
      }
      ...
      $list_user_id[]         = intval($value['inspection_by']);
      $list_user_id[]         = intval($value['inspection_client_by']);
    }
    ...
    foreach($key_list['combined'] as $value) {

      if(isset($detail_mv[$value])) {
        $data_detail                = $detail_mv[$value];

        if($total_approved[$value] == count($data_detail)) {
      ...
      $data['detail_material']  = $data_detail;
      $data['main_material']    = $data_detail[0];

      $file_name                = $data['report_no_format'][$data['main_material']['project_code']][$data['main_material']['discipline']][$data['main_material']['module']][$data['main_material']['type_of_module']]['mv_no'] . '-' . $data_detail[0]['report_number'];

      // $html                     = $this->load->view('material_verification/pdf/material_verification_pdf', $data, true);
      $html                     = $this->load->view('mdb/offline/material_verification_pdf', $data, true);
  ...
  public function additional_attachment($deck = null, $action = "view", $offline_loc = null, $id_type = null, $sftp = null) {
    ...
    foreach($datadb as $key => $value) {
			...
			if(@count($output[$value['id_type']]) > $this->limit) {
        continue;
      }
      $this->ecodoc_no_list[] = $value['ecodoc_no'];
      if (file_exists($offline_loc.'/'.$value['attachment_name'])) continue;
  ...
  public function offline($deck = null) {
    ...
    if(!is_dir($dir_home)) {
      mkdir($dir_home, $permission, $recursive);
    }

    // $index_b                        = "$dir_home"."MDB/MDB INDEX B";
    $index_b                        = "$dir_home"."src";
    $assets_loc                     = $dir_home."assets";
    ...
    foreach($deck_list as $value) {
      $start_time_deck              = microtime(true);
      $dir_deck_name                = "mdb_index_b_".str_replace(" ","_", strtolower($value['name']));

      @mkdir($index_b.'/'.$dir_deck_name, $permission, $recursive);
      $deck_loc                     = $index_b.'/'.$dir_deck_name;
      ...
      foreach($mdb_general_volume_list as $volume) {
        ...
        if(isset($mdb_general_section_list[$volume['volume']])) {
          foreach($mdb_general_section_list[$volume['volume']] as $section) {
            ...
            $params[$value['id']]['link_'.$section['var_code']] = "src/$dir_deck_name/$second_level_ori";
          }
        }
      }

      $params[$value['id']]['offline']              = true;
      $params[$value['id']]['index_loc']            = $dir_home;
      $params[$value['id']]['index_name']           = $dir_deck_name;

      $end_time_deck                                = microtime(true);
      $execution_time_deck                          = ($end_time_deck - $start_time_deck)/60;
      $execution_time_deck                          = round($execution_time_deck, 2);

      $deck_time[$value['id']]                      = [
        'deck_name'                                 => $value['name'],
        'time'                                      => $execution_time_deck. ' Minutes',
        'dir_size'                                  => $this->getDirectorySize($deck_loc)
      ];
    }
    ...
    if($list_ecodoc_no) {
      ...
      foreach($mdr_list as $value) {
        ...
        $this->ecodoc_no_list[$value['client_doc_no']]  = 'src/ecodoc/'.$value['client_doc_no'].'.'.$ext;
      }
    }
    ...
    $total_all_size               = 0;
    foreach ($deck_time as $v) {
      echo $v['deck_name'].' Execution Time : '.$v['time']. ', Dir Size : '.$v['dir_size']. ' MB';
      echo '<br>';

      $total_all_size += $v['dir_size'];
    }
    echo "<br>Overall Execution Time : $execution_time Minutes <br>";
    echo 'Overall Size : '.$total_all_size.' MB';
  ...
  protected function GetDirectorySize($path){
    ...
    $dir_size   = round($bytestotal / 1024 / 1024,2);
    return $dir_size;
  }
  ...
  protected function visual_pdf($report_number = Null, $access = Null, $drawing_no = Null, $postpone_reoffer_no = NULL, $file_loc = null){
    ...
    foreach ($datadb as $key => $value) {
      $master_report_no[$value['discipline']][$value['module']][$value['type_of_module']][$value['category']] = $value['report_no'];
		...
		$welder_ref_rh_s = array_filter(array_column($data['visual_report'], 'welder_ref_rh'));
		foreach ($welder_ref_rh_s as $key => $value) {
			foreach (explode(';', $value) as $keys => $values) {
				$welder_ref[] = $values;
			}
		}
		$welder_ref_fc_s = array_filter(array_column($data['visual_report'], 'welder_ref_fc'));
		foreach ($welder_ref_fc_s as $key => $value) {
			foreach (explode(';', $value) as $keys => $values) {
				$welder_ref[] = $values;
			}
		...
		if($id_welder){
			$datadb = $this->visual_mod->master_welder_new(["id_welder IN (".implode(', ', $id_welder).")" => NULL]);
			foreach ($datadb as $key => $value) {
				$data['master_welder'][$value['id_welder']] = $value;
			}
		}
  ...
  public function irn_pdf($submission_id = null, $report_format = null,$file_loc = null){
    $enc_submission_id = $submission_id;
    ...
    // test_var([$report_format, $file_loc, $data['drawing_unique']]);

    // TEMPORARY GENERATE WTR HTML LINK FROM DRAWING IRN

    if($data['drawing_unique']) {
      $wtr_html_loc = $file_loc.'/'.$report_format;
      if(!is_dir($wtr_html_loc)) {
        mkdir($wtr_html_loc, 0777, true);
      }


      $project_enc 				= encrypt($data['show_pcms_irn'][0]['project_id']);
      $drawing_type_enc 	= encrypt($data['show_pcms_irn'][0]['drawing_type']);
      $discipline_enc 		= encrypt($data['show_pcms_irn'][0]['discipline']);
      $module_enc				  = encrypt($data['show_pcms_irn'][0]['module']);
      $type_of_module_enc = encrypt($data['show_pcms_irn'][0]['type_of_module']);

      $params_html        = [];


        foreach($data['drawing_unique'] as $value) {
          $drawing_no_enc   = encrypt($value);
        ...
        if($params_html) {
          $this->irn_wtr_html_new($enc_submission_id, $params_html, $wtr_html_loc, null);
        }
  ...
  public function irn_wtr_html_new($submission_id, $params, $offline_loc, $is_piping = null){
    error_reporting(0);
    $submission_id = $this->encryption->decrypt(strtr($submission_id, '.-~', '+=/')); 
    ...
    //   $where['b.drawing_no'] = $drawing_no;
    // }

    $where[implode_where("b.drawing_no", $params['drawing_no'])]  = null;
    $where['a.submission_id'] = $submission_id;
    $datadb_irn 				= $this->irn_mod->show_pcms_irn($where);
    unset($where);

    if($datadb_irn) {
      foreach($datadb_irn as $value) {
        // PASS 1
        $show_pcms_irn[$value['drawing_no']][] = $value;
        $list_user_id[]           = intval($value['smoe_approval_by']);
        $list_user_id[]           = intval($value['client_approval_by']);
      }

      $where_user["id_user IN ('".implode("', '", $list_user_id)."')"] = NULL;   
    ...
    if(isset($submission_id) && $submission_id != "wtr"){ 
    $where['e.submission_id'] = $submission_id; 
    }
    $result_db =  $this->irn_mod->show_data_irn_joint($where); 
    unset($where);   
    ...
    foreach($result_db as $value) {
      // PASS 2
      $result[$value['drawing_no']][] =  $value;
      $id_joint[]                     = intval($value['id']);
    }

    // PASS 3
    // $data["show_data_irn_list_filter"] = array($result[0]);  
  
    $document_unique = array_unique($id_joint); 
    $where["a.id IN ('".implode("', '", $document_unique)."')"] = NULL;
      
    $wtr_db   = $this->wtr_mod->wtr_all_of_joint_list($where);	
    unset($where); 

    $piecemark_list_submit_1 = array(); 
    $piecemark_list_submit_2 = array(); 

    foreach($wtr_db as $value) {
      // PASS 4
      $wtr_list[$value['drawing_no']][] = $value;
      $drawing_nox[]   	  = $value['drawing_no']; 
      $drawing_no_wmx[]   = $value['drawing_wm']; 
      $piecemark_list_submit_1[] =  $value['pos_1']; 
      $piecemark_list_submit_2[] =  $value['pos_2']; 
    }
    ...
    //  PASS 5
      ...
      $datadb  = $this->fitup_mod->piecemark_list_with_itr($wherex);  
      foreach ($datadb as $key => $value) { 
        if(!isset($data['status_piecemark'][$value['part_id']]) || ($value['id_mis'] != NULL && $data['status_piecemark'][$value['part_id']]['id_mis'] = NULL)){
          $data['status_piecemark'][$value['part_id']] = $value; 
          $data['status_piecemark_ref_1'][$value['id']] = $value; 
      ...
      if(sizeof(array_filter(array_column($data['status_piecemark'], 'id_mis'))) > 0){

        $id_mis     = array_filter(array_column($data['status_piecemark'], 'id_mis'));  
        $where["id_mis_det IN ('".implode("', '", $id_mis)."')"] = NULL; 
        $datadb = $this->fitup_mod->warehouse_mis_mrir($where);
        unset($where); 
        foreach ($datadb as $key => $value) {
      ...
      $where['project_id']   	 = $wtr_db[0]['project'];
      ...
      $data['meta_title'] 	 = ($submission_id == "wtr" ? "PCMS - MWTR" : "IRN Inspection");	
      if($is_piping){
        // $html = $this->load->view('irn/irn_pdf_joint_landscape_pp',$data);	
      }	else {
      ...
      $data['meta_title'] 	 = ($submission_id == "wtr" ? "PCMS - MWTR" : "IRN Inspection");			
      // $html = $this->load->view('irn/irn_pdf_joint_landscape',$data);	
      foreach($params['drawing_no'] as $value) {

        $data['show_pcms_irn']              = $show_pcms_irn[$value];
        $data['result']                     = $result[$value];
        $data["show_data_irn_list_filter"]  = array($data['result'][0]);

        $data['wtr_list']                   = $wtr_list[$value];
        $data['joint_list']                 = $data['wtr_list'];

        $html_name	= $offline_loc.'/'.$value.'.html';

        $myfile   	= fopen($html_name, "w");
        $view 			= $this->load->view('mdb/offline/irn_wtr', $data, true);  
  ...
  public function irn_wtr_html($submission_id = null,$drawing_no = null, $is_piping = NULL, $offline_loc = null){
		error_reporting(0);
		$submission_id = $this->encryption->decrypt(strtr($submission_id, '.-~', '+=/')); 
    ...
    if($data['wtr_list'][0]['is_bondstrand']==1){
      $drawenc = strtr($this->encryption->encrypt($data['wtr_list'][0]['drawing_no']),'+=/', '.-~');
      redirect('btr/detail_btr/'.$drawenc);
    }
      ...
      $html_name	= $offline_loc.'/'.$drawing_no.'.html';
  ...
  function fitup_pdf($project,$discipline,$module,$type_of_module,$report_number=null,$submission_id = null,$revise = null,$company_id = null, $file_loc = null){
		...
		$fitter_id_s = array_filter(array_column($data['joint_list'], 'fitter_id'));
		foreach ($fitter_id_s as $key => $value) {
			foreach (explode(';', $value) as $keys => $values) {
				$fitter_id[] = intval($values);
			}
		...
		if($fitter_id){
			$datadb = $this->fitup_mod->fitter_code(["id_fitter IN (".implode(', ', $fitter_id).")" => NULL]);
			foreach ($datadb as $key => $value) {
				$data['fitter_code_arr'][$value['id_fitter']] = $value['fit_up_badge'];
			}
		}

		$tack_weld_id_s = array_filter(array_column($data['joint_list'], 'tack_weld_id'));
		foreach ($tack_weld_id_s as $key => $value) {
			foreach (explode(';', $value) as $keys => $values) {
				$welder_ref[] = $values;
			}
		}
		...
		if($id_welder){
			$datadb = $this->fitup_mod->welder_code_version_view_only(["a.id_welder IN (".implode(', ', $id_welder).")" => NULL]);
			foreach ($datadb as $key => $value) {
  ...
  public function material_certificate() {
      ...
      foreach($data_list as $value) {
        ...
        if(isset($certificate[$value['receiving_id']][$value['mill_cert_no']])) {
          ...
          if(strpos($cat_name, 'PLATE') !== false) {
						if(@count($output["certificate_plate"]) > $this->limit) {
							continue;
						}
            $output["certificate_plate"][]    = $row;
          } elseif(strpos($cat_name, 'PIPE') !== false) {
						if(@count($output["certificate_pipe"]) > $this->limit) {
							continue;
						}
            $output["certificate_pipe"][]    = $row;
          } else {
						if(@count($output["certificate_others"]) > $this->limit) {
							continue;
						}
    ...
    $index_name                   = $params['index_loc'].'/mdb_index_b_jacket.html';
    $myfile                       = fopen($index_name, "w");
    $data['params']               = $params;
  ...
  public function fitup_jacket_all($action = "view", $offline_loc = null){
    ...
    foreach ($datadb as $key => $value) {
      ...
      if($action == "save" AND !file_exists($offline_loc.'/'.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['fitup_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf')) {
        $this->fitup_pdf(
      ...
      } elseif($action == "save" AND file_exists($offline_loc.'/'.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['fitup_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf')){
      	test_var('skiped file '.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['fitup_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf'.' already existed!', 1);
      }  
  ...
  public function mv_jacket_all($action = "view", $offline_loc = null){
    ...
    if($data_list) {
      foreach($data_list as $key => $value) {
        ...
        if($action == "save" AND !file_exists($offline_loc.'/'.$this->report[$value['project_code']][$value['discipline']][$value['module']][$value['type_of_module']][$rep_cat].'-'.$value['report_number'].'.pdf')){
        ...
        } elseif($action == "save" AND file_exists($offline_loc.'/'.$this->report[$value['project_code']][$value['discipline']][$value['module']][$value['type_of_module']][$rep_cat].'-'.$value['report_number'].'.pdf')){
          $file_loc                       = encrypt($offline_loc);
          test_var('skiped file '.$this->report[$value['project_code']][$value['discipline']][$value['module']][$value['type_of_module']][$rep_cat].'-'.$value['report_number'].'.pdf'.' already existed!', 1);
        }
  ...
  public function irn_jacket_all($action = "view", $offline_loc = null){ 
    ...
    foreach ($datadb as $key => $value) {
      ...
      if($action == "save" AND !file_exists($offline_loc."/".$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['irn_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf')) { 
      ...
      } elseif($action == "save" AND file_exists($offline_loc."/".$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['irn_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf')) {
        test_var('skiped file '.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['irn_report'.($value['company_id']==13 ? '_scm' : '')].$value['report_number'].'.pdf'.' already existed!', 1);
      }
    }
    return $output;
  }
  ...
  public function ndt_jacket_all($action = "view", $offline_loc = null, $ndt_type = null, $sftp = null){
    ...
    foreach ($datadb as $key => $value) {
      ...
      if($action == "save" AND !file_exists($offline_loc."/".$value['report_number'].'.pdf')) {
      	$this->ecodoc_no_list[] = $value['ecodoc_no'];

        $local_file       = $offline_loc."/".$value['report_number'].'.pdf';
        $remote_file      = "/PCMS/NDT/upload/ndt/".$value["filename"];
        $sftp->get($remote_file, $local_file);
      } elseif($action == "save" AND file_exists($offline_loc."/".$value['report_number'].'.pdf')) {
        test_var('skiped file '.$value['report_number'].'.pdf'.' already existed!', 1);
      }
  ...
  public function visual_jacket_all($action = "view", $offline_loc = null){
    ...
    foreach ($datadb as $key => $value){
      ...
      if($action == "save" AND !file_exists($offline_loc.'/'.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['visual_report'.($value['company_id']==13 ? '_13' : '')].$value['report_number'].'.pdf')) {
      ...
      } elseif($action == "save" AND file_exists($offline_loc.'/'.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['visual_report'.($value['company_id']==13 ? '_13' : '')].$value['report_number'].'.pdf')){
        $file_loc                       = encrypt($offline_loc);
        test_var('skiped file '.$this->report[$value['project']][$value['discipline']][$value['module']][$value['type_of_module']]['visual_report'.($value['company_id']==13 ? '_13' : '')].$value['report_number'].'.pdf'.' already existed!', 1);
      }
  ...
  public function wtr_jacket_all($action = "view", $offline_loc = null) {   	
    ...
    foreach($datadb as $key => $value) {
      ...
      if($action == "save" AND !file_exists($offline_loc.'/'.$value['drawing_no'].'.html')) {
	      if(@!in_array($value['project'], $params['project'])) {
	        $params['project'][]        = $value['project'];
	      }
	      if(@!in_array($value['drawing_no'], $params['drawing_no'])) {
	    ...
	    } elseif($action == "save" AND file_exists($offline_loc.'/'.$value['drawing_no'].'.html')){
      	test_var('skiped file '.$value['drawing_no'].'.html'.' already existed!', 1);
      }
    ...
    if($action == "save" && $params) {
      $this->wtr_html_new($params, $offline_loc);
    }
  ...
  public function wtr_jacket($action = "view", $offline_loc = null) {
  ...
  public function master_mdb_ecodoc_list() {
    ...
    if(!is_dir($this->drive)) {
      @mkdir($this->drive, $permission, $recursive);
    }

    $index_b                        = $this->drive."/src";
    $ecodoc_loc                     = $index_b."/ecodoc";

    // $index_b                        = "C:\mdb_offline\src\mdb_index_b_jacket";
    // $ecodoc_loc                     = "C:\mdb_offline\src\mdb_index_b_jacket\ecodoc";
    ...
    @mkdir($index_b.'/'.'mdb_index_b_jacket', $permission, $recursive);
    $deck_loc                     = $index_b.'/'.'mdb_index_b_jacket';
    $dir_main                     = $deck_loc;
      ...
      if(isset($mdb_general_section_list[$volume['volume']])) {
        foreach($mdb_general_section_list[$volume['volume']] as $section) {
          ...
          @mkdir($second_level, $permission, $recursive);
            ...
            echo "======================================";
    				$start_time_sub = microtime(true);

            $this->mv_jacket_all('save', $second_level);
            $size = $this->GetDirectorySize($second_level);

            $end_time_sub    = microtime(true);
				    $execution_time  = ($end_time_sub - $start_time_sub)/60;
				    $execution_time  = round($execution_time, 2);
            test_var($second_level.' size is <b>'.$size.' MB</b>', 1);
            test_var(' load on <b>'.$execution_time.' Minutes</b>', 1);
            echo "======================================";

          } else if(in_array($section['var_code'], ['mv_dossier_19', 'mv_dossier_22', 'mv_dossier_29']) ){
          ...
          } else if($section['var_code'] == "visual_structure") {
          ...
          } else if(in_array($section['var_code'], ['visual_dossier_19', 'visual_dossier_22', 'visual_dossier_29']) ){
          ...
          } else if($section['var_code'] == "fitup_structure") {
          ...
          } else if(in_array($section['var_code'], ['fitup_dossier_19', 'fitup_dossier_22', 'fitup_dossier_29']) ){
          ...
          } else if(in_array($section['var_code'], ['ndt_structure_3', 'ndt_structure_1', 'ndt_structure_2', 'ndt_structure_7']) ){
          ...
          } else if(in_array($section['var_code'], ['ndt_dossier_19_3', 'ndt_dossier_19_1', 'ndt_dossier_19_2', 'ndt_dossier_19_7','ndt_dossier_22_3', 'ndt_dossier_22_1', 'ndt_dossier_22_2', 'ndt_dossier_22_7','ndt_dossier_29_3', 'ndt_dossier_29_1', 'ndt_dossier_29_2', 'ndt_dossier_29_7',]) ){
          ...
          } else if(in_array($section['var_code'], ['additional_att_223', 'additional_att_231', 'additional_att_221', 'additional_att_224', 'additional_att_234', 'additional_att_233', 'additional_att_229', 'additional_att_230', 'additional_att_227', 'additional_att_232', 'additional_att_222'])) {
          ...
          } else if(in_array($section['var_code'], ['additional_att_225', 'additional_att_226', 'additional_att_228'])) {
          ...
          } else if(in_array($section['var_code'], ['dimension_all'])) {
          ...
          } else if(in_array($section['var_code'], ['irn_structure'])) {
          ...
          } else if(in_array($section['var_code'], ['irn_dossier_19', 'irn_dossier_22', 'irn_dossier_29'])) {
          ...
          } else if(in_array($section['var_code'], ["wtr_structure"])) {

          	echo "======================================";
      			$start_time_sub    = microtime(true);
            
            $this->wtr_jacket_all("save", $second_level);
          	$size = $this->GetDirectorySize($second_level);
            $end_time_sub    = microtime(true);
				    $execution_time  = ($end_time_sub - $start_time_sub)/60;
				    $execution_time  = round($execution_time, 2);
          ...
          } else if(in_array($section['var_code'], ['wtr_dossier_19', 'wtr_dossier_22', 'wtr_dossier_29'])) {
            
            echo "======================================";
      			$start_time_sub    = microtime(true);
            
            $this->wtr_jacket("save", $second_level);
            $size = $this->GetDirectorySize($second_level);
            $end_time_sub    = microtime(true);
				    $execution_time  = ($end_time_sub - $start_time_sub)/60;
				    $execution_time  = round($execution_time, 2);
          ...
          foreach ($mdb_general_subsection_list[$volume['volume']][$section['section']] as $key_subsec => $value_subsec) {
            ...
            @mkdir($third_level, $permission, $recursive);
              ...
              $this->shopdrawing_dossier_jacket('save', $third_level, $dr_type, $sftp, $desc_assy);
              unset($desc_assy);
              $size = $this->GetDirectorySize($third_level);
	            test_var($third_level.' size is <b>'.$size.' MB</b>', 1);
          	...
          	$params['link_'.$value_subsec['var_code']] = "src/mdb_index_b_jacket".$third_level_ori;
          }
          $params['link_'.$section['var_code']] = "src/mdb_index_b_jacket".$second_level_ori;
        }
      }
    }
    $list_ecodoc_no                   = array_filter(array_unique($this->ecodoc_no_list));
    ...
    if($list_ecodoc_no) {
      ...
      foreach($mdr_list as $value) { 
        ...
        if(!file_exists($local_file)){
          ...
          test_var('skiped file '.$value['client_doc_no'].'.'.$ext.' already existed!', 1);
        }
        $this->ecodoc_no_list[$value['client_doc_no']]  = 'src/ecodoc/'.$value['client_doc_no'].'.'.$ext;
      }
      $start_time_sub    = microtime(true);
      
      $size = $this->GetDirectorySize($ecodoc_loc);
      test_var($ecodoc_loc.' size is <b>'.$size.' MB</b>', 1);
    ...
    error_reporting(0);
    test_var("DONE!, total loading time is ".$execution_time);
    return;
	...
	public function wtr_html($project,$drawing_no,$drawing_type,$discipline,$module,$type_of_module, $offline_loc = null){
    error_reporting(0);
		$project 		= $this->encryption->decrypt(strtr($project, '.-~', '+=/'));
		$drawing_no 	= $this->encryption->decrypt(strtr($drawing_no, '.-~', '+=/'));
		...
		// Fitter Code
			$fitter_id_s = array_filter(array_column($data['wtr_list'], 'fitter_id'));
			foreach ($fitter_id_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$fitter_id[] = intval($values);
				}
		...
		// Welder Code
			$welder_ref_rh_s = array_filter(array_column($data['wtr_list'], 'welder_ref_rh'));
			foreach ($welder_ref_rh_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$welder_ref[] = $values;
				}
			}
			$welder_ref_fc_s = array_filter(array_column($data['wtr_list'], 'welder_ref_fc'));
			foreach ($welder_ref_fc_s as $key => $value) {
				foreach (explode(';', $value) as $keys => $values) {
					$welder_ref[] = $values;
				}
    ...
    @mkdir($assets_loc, $permission, $recursive);
		...
		//COPYING ASSETS FAVICON
		copy('img/favicon.png', $assets_loc.'favicon.png');
    ...
    // COPYING ASSETS FONT AWESOME

    @mkdir($assets_loc.'css', $permission, $recursive);
    @mkdir($assets_loc.'webfonts', $permission, $recursive);


    $fa_file_1                      = scandir('assets/fontawesome-free/css');
    $fa_file_1                      = array_diff($fa_file_1, array('.', '..'));

    foreach($fa_file_1 as $value) {
      copy('assets/fontawesome-free/css/'.$value, $assets_loc.'css/'.$value);
    }

    $fa_file_2                      = scandir('assets/fontawesome-free/webfonts');
    $fa_file_2                      = array_diff($fa_file_2, array('.', '..'));

    foreach($fa_file_2 as $value) {
      copy('assets/fontawesome-free/webfonts/'.$value, $assets_loc.'webfonts/'.$value);
    }

		$this->session->set_flashdata('success', 'MDB Assets has been completed!');
		redirect($_SERVER["HTTP_REFERER"]);
	}

	public function mdb_general_offline($data){
		$data['assets'] = [
    ...
    $dir_home = $this->drive;
      ...
      mkdir($dir_home, $permission, $recursive);
		...
		$index_b = "$dir_home"."src/mdb_index_a/";
		$ecodoc_loc = "$dir_home"."src/ecodoc/";
    ...
    $sftp->login($ftp['username'], $ftp['password']);
		...
		// 	if(strpos($cat_name, 'certificate_') !== false) {
		// 		$this->download_file_ecodoc($sftp, $ecodoc_loc, $value);
		// 	}
		// }

		foreach($data['mdb_general_volume_list'] as $volume) {
			$list_ecodoc_no[]						= $volume['ecodoc_no'];
			$first                      = $volume['volume'].'. '.$volume['document_description'];
			$first_level                = $index_b.$first.'/';

			if(isset($data['file_list'][$volume['var_code']])){
				@mkdir($first_level, $permission, $recursive);
			}

			foreach($data['mdb_general_section_list'][$volume['volume']] ?? [] as $section) {
				$list_ecodoc_no[]				= $section['ecodoc_no'];
				$second                 = $section['volume'].'.'.$section['section'].'. '.$section['document_description'];
				$second_level           = $first_level.$second.'/';

				if(isset($data['file_list'][$section['var_code']])){
					@mkdir($second_level, $permission, $recursive);
				}

				foreach($data['mdb_general_subsection_list'][$section['volume']][$section['section']] ?? [] as $subsection) {
					$list_ecodoc_no[]			 = $subsection['ecodoc_no'];
					$third                 = $subsection['volume'].'.'.$subsection['section'].'.'.$subsection['subsection'].' '.$subsection['document_description'];
					$third_level           = $second_level.$third.'/';
	
					if(isset($data['file_list'][$subsection['var_code']])){
						@mkdir($third_level, $permission, $recursive);
						
						if(strpos($subsection['var_code'], 'certificate_') !== false) {
							$file_loc_link = str_replace($dir_home, '', $third_level) ;
							$list_ecodoc_no = array_unique(array_merge($list_ecodoc_no, array_column($data['file_list'][$subsection['var_code']], 'ecodoc_no')));
							$data['file_list'][$subsection['var_code']] = $this->download_file_certificate($sftp, $third_level, $file_loc_link, $data['file_list'][$subsection['var_code']]);
						}
					}
				}
			}
		...
		if (($key = array_search('', $list_ecodoc_no)) !== false) {
			unset($list_ecodoc_no[$key]);
		}
		if(count($list_ecodoc_no) > 0){
			$list_ecodoc_no = array_unique($list_ecodoc_no);
			$file_loc_link = str_replace($dir_home, '', $ecodoc_loc) ;
			$this->download_file_ecodoc($sftp, $ecodoc_loc, $file_loc_link, $list_ecodoc_no);

			foreach($data['mdb_general_volume_list'] as $volume_key => $volume) {
				$volume['link_ecodoc'] = $this->temp_link[$volume['ecodoc_no']] ??'#';
				$data['mdb_general_volume_list'][$volume_key] = $volume;
	
				foreach($data['mdb_general_section_list'][$volume['volume']] ?? [] as $section_key => $section) {
					$section['link_ecodoc'] = $this->temp_link[$section['ecodoc_no']] ??'#';
					$data['mdb_general_section_list'][$section['volume']][$section_key] = $section;
	
					foreach($data['mdb_general_subsection_list'][$section['volume']][$section['section']] ?? [] as $subsection_key => $subsection) {
						$subsection['link_ecodoc'] = $this->temp_link[$subsection['ecodoc_no']] ??'#';
						$data['mdb_general_subsection_list'][$subsection['volume']][$subsection['section']][$subsection_key] = $subsection;
					}
				}
			}
		}

    $index_name                   = $dir_home.'/mdb_index_a.html';
    $myfile                       = fopen($index_name, "w");
    $view                         = $this->load->view($data['subview'], $data, true);
    fwrite($myfile, $view);
	...
	public function download_file_certificate($sftp, $location, $file_loc_link, $file_list){
		foreach ($file_list as $key => $value) {
			$local_file       = $location.$value['attachment'];
			$remote_file      = "/PCMS/warehouse/receiving/".$value["attachment"];
			if(!file_exists($local_file)){
				if(!$sftp->get($remote_file, $local_file)){
				}
			}
			if(file_exists($local_file)){
				$value['link'] = $file_loc_link.$value['attachment'];
			}
	...
	public function download_file_ecodoc($sftp, $location, $file_loc_link, $ecodoc_list){
		$where[implode_where("client_doc_no", $ecodoc_list)] = null;
		$where['mrd.attachment IS NOT NULL']  = null;
		$datadb                           = $this->mdb_mod->mdr_document_list($where);
		unset($where);
		...
		foreach($mdr_list as $value) {
			$ext              = pathinfo($value['attachment'], PATHINFO_EXTENSION);
			$local_file       = $location.$value['client_doc_no'].'.'.$ext;
			$remote_file      = "/PCMS/pcms_ori/upload/production_design/file/".$value["attachment"];
			if(!file_exists($local_file)){
				if(!$sftp->get($remote_file, $local_file)){
				}
			...
			if(file_exists($local_file)){
				$this->temp_link[$value['client_doc_no']]  = $file_loc_link.$value['client_doc_no'].'.'.$ext;
			}
		}
	}

	public function mdb_dashboard_offline(){
		$dir_home = $this->drive;
    $permission = 0777;
    $recursive = true;
      ...
      mkdir($dir_home, $permission, $recursive);
		...
		$data['meta_title']         = 'MDB Dashboard';
    $data['subview']            = 'mdb/offline/mdb_dashboard_offline';
		
		$index_name                   = $dir_home.'/index.html';
    $myfile                       = fopen($index_name, "w");
    $view                         = $this->load->view($data['subview'], $data, true);
    fwrite($myfile, $view);
