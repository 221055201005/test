...
class Dimension extends CI_Controller
{
	...
	{
		redirect('dimension/dc_list/' . strtr($this->encryption->encrypt($type_of_report), '+=/', '.-~'));
	}
	...
	function draw_list_filter()
	{
		...
		foreach ($list as $list) {
			...
			$row[] = "<a href='" . $links . "' class='btn btn-primary text-white' title='Detail'><i class='fas fa-plus-circle'></i> Add</a>";
	...
	function dc_list($type_of_report = NULL)
	{
		...
		$datadb  = $this->general_mod->portal_user_get_db_new();
		foreach ($datadb as $value) {
			$data['user_list'][$value['id_user']] = $value['full_name'];
		}
		// test_var("ASD");
		...
		if (empty($_FILES['file_attch_1']['name'])) { //KALAU FILE ATTACH KOSONG
			...
			foreach ($id_file_attch as $id_file) {
				...
				if (!$this->upload->do_upload('file_attch_' . $id_file)) {
					...
					redirect('dimension/dc_list/draft');
					return false;
				}
	...
	function dc_approval_proccess()
	{
		...
		if ($status_approval == 3) {
			...
			redirect('dimension/manual_sign/' . $module . '/' . $user_id . '/' . $report_no . '/' . $id);
			...
			redirect('dimension/dc_list/' . $str_status);
		}
	}
	...
	public function delete_submission_additional_report($submission_id)
	{
		...
		$this->dimension_mod->additional_report_delete($submission_id, $data_update);
		unset($where);
		unset($data_update);
	...
	function additional_report($type_of_report = NULL)
	{
		...
		} else if ($type_of_report == 5) {
			$meta = "Thickness Gauge Reports";
		} else {
			$meta = "-";
		}
		$data['meta_title'] 	 = $meta;
	...
	function save_additional_report()
	{
		...
		if ($data["post"]["count_checked"] > 0) {
			...
			foreach ($data["post"]["check_status"] as $key => $value) {
				if ($value == 1) {

					$data_file = array(
						"id_joint"		 		=> $data["post"]["id_joint_temp"][$key],
	...
	public function create_new_rfi($drawing_no = null){
    ...
    $data['user_list']     = $this->general_mod->portal_user_db_no_sign([
  	...
  	$data['subview']      = 'dimension/create_new_rfi';
    $data['meta_title']   = 'Create New RFI Details';
    $data['sidebar']      = $this->sidebar;
    $this->load->view('index', $data);
  }

	function create_new_rfi_process() {
		...
		$data_dimension = array(
			...
			'requestor_company' => $post['company'],
			'option_date' 		=> $post['submitted_date'],
			'result'			=> 0
		);
		$this->dimension_mod->dimension_control_add($data_dimension);
		...
		$id_dc_atc = $this->dimension_mod->dimension_control_add_attch($form_data);
	...
	function rfi_detail($id_dc_atc) {
		$id_dc_atc = decrypt($id_dc_atc);

		$datadb = $this->dimension_mod->data_dc_attch([
			"id" => $id_dc_atc,
		]);
		if(count($datadb) == 0){
			redirect('dimension/dc_list');
		...
		$datadb = $this->dimension_mod->data_draw_dc([
			"submission_id" => $dc_atc['submission_id'],
		]);
		$dc = $datadb[0];
		...
		if(count($datadb) == 0){
			$form_data = [
				'id_main' => $id_dc_atc,
				'category' => 6,
				'rfi_no' => $dc_atc['rfi_no'],
				'drawing_no' => $dc['drawing_no'],
				'project' => $dc_atc['project_id'],
				'discipline' => $dc['discipline'],
				'module' => $dc['module'],
				'type_of_module' => $dc['type_of_module'],
				'deck_elevation' => $dc['deck_elevation'],
				'company_id' => $dc['requestor_company'],
				'submitted_date' => $dc['option_date'],
				'remarks' => $dc_atc['remarks'],
				'created_by' => $dc_atc['uploaded_by'],
			];
			$this->dimension_mod->rfi_additional_report_insert_process($form_data);
    ...
    $data['user_list']     = $this->general_mod->portal_user_db_no_sign([
		...
		$data['subview']      = 'dimension/rfi_detail';
	...
	function update_rfi_process() {
		...
		$this->dimension_mod->pcms_dimension_control_update_process($data_dimension, [
			"id" => $post['id_dc'],
		]);
		...
		if($fileName != ''){
			...
			$form_data['uploaded_by'] = $this->user_cookie[0];
			$form_data['upload_on'] = date('Y-m-d H:i:s');
		}
		$this->dimension_mod->pcms_dimension_control_attach_update_process($form_data, [
			"id" => $post['id_dc_atc'],
		]);
	...
	function rfi_detail_delete_process($id_rfi_detail){
		$id_rfi_detail = decrypt($id_rfi_detail);
		...
		$this->session->set_flashdata('success', 'Data deleted successful!');
		redirect($_SERVER['HTTP_REFERER']);
	}

	function rfi_pdf($id_dc_atc) {
		$id_dc_atc = decrypt($id_dc_atc);
		...
		$sign_list = [
			"contractor" => [
				"id_user" => $dc_atc['uploaded_by'],
				"date" => $dc_atc['upload_on'],
			],
			$client_text => [
				"id_user" => NULL,
				"date" => NULL,
		...
		// test_var($data);
		// $this->load->view('dimension/rfi_pdf', $data);

		$this->load->library('Pdfgenerator_client');
		$html = $this->load->view('dimension/rfi_pdf', $data, true);
		$this->pdfgenerator_client->generate($html, $dc_atc['report_number'], NULL);
	}

	function send_mail_notif_process($id_dc_atc){
		$id_dc_atc = decrypt($id_dc_atc);
		...
		$error = '';
		if($rfi['inspector_id'] == ''){
			$error = 'Inspector Name is Blank';
		}
		elseif($rfi['submitted_date'] == ''){
			$error = 'Submitted Date is Blank';
		}
		elseif($rfi['inspection_date'] == ''){
			$error = 'Inspect Date from is Blank';
		}
		elseif($rfi['inspection_date_to'] == ''){
			$error = 'Inspect Date to is Blank';
		}
		elseif($rfi['area'] == ''){
			$error = 'Inspect Area is Blank';
		}
		elseif($rfi['location'] == ''){
			$error = 'Inspect Location is Blank';
		}
		elseif($rfi['itp_authority'] == ''){
			$error = 'ITP Intervention to Client is Blank';
		}
		elseif($rfi['type_of_inspection'] == ''){
			$error = 'Type of Inspection is Blank';
		}
		elseif(count($rfi_detail_list) == 0){
			$error = 'No Item Number Found at RFI';
		}
		if($error != ''){
			$this->session->set_flashdata('error', $error);
			redirect($_SERVER['HTTP_REFERER']);
		...
		$form_data = [
			'send_notif_date' => date('Y-m-d H:i:s'),
			'send_notif_by' => $this->user_cookie[0],
		];
		$this->dimension_mod->rfi_additional_report_update_process($form_data, [
			"id" => $rfi['id']
		]);
		...
		if(in_array(3, $itp_authority) || in_array(4, $itp_authority)){
			$this->session->set_flashdata('success', 'Mail will send at 12 PM');
			redirect($_SERVER['HTTP_REFERER']);
		}
		else{
