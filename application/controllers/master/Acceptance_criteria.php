...
class acceptance_criteria extends CI_Controller
{
  ...
  public function acceptance_criteria_list($download = null)
  {
    ...
    foreach ($datadb as $key => $value) {
      $data['type_of_module_list'][$value['id']] = $value;
    }
    
    $post = $this->input->post();
    if (@$post['project_id'] != '') {
      $where['project'] = @$post['project_id'];
    }
    if (@$post['discipline'] != '') {
      $where['discipline'] = @$post['discipline'];
    }
    $where['status_delete'] = 1;
    $data['acceptance_criteria'] = $this->general_mod->acceptance_criteria($where);
    unset($where);
    $data['meta_title']     = 'Acceptance Criteria Register';
    $data['subview']        = 'Master/Acceptance_criteria/acceptance_criteria_list';
    $this->load->view('index', $data);
  }
  
  public function acceptance_criteria_new($id = null)
  {
    $datadb = $this->general_mod->project();
    foreach ($datadb as $key => $value) {
      $data['project_list'][$value['id']] = $value;
    ...
    $data['meta_title']   = ucfirst($module) . ' Accpetance Criteria';
    $data['subview']      = 'master/acceptance_criteria/acceptance_criteria_new';
    $this->load->view('index', $data);
  }

  public function acceptance_criteria_save_process()
  {
